...
function pl(e) {
	return ["<p>Generated by Drowsy's WFRP4e Customizers. This Trait applies a custom Species Builder wound formula.</p>", `<p>${gl(e.formula)}</p>`].join("");
}
function ml(e) {
...
}), $ = "wfrp4e-customizer-apps", yw = "Drowsy's WFRP4e Customizers", bw = "wfrp4e", xw = Gs();
//#endregion
...
function Sw(e) {
	...
	if (!t) return "";
	if (Aw(t)) return t;
	let n = Ew(t), r = Ow(n, e.documentType);
	return r ? jw(n) ? JSON.stringify({
		type: r,
		uuid: n
	}) : JSON.stringify({
		id: n,
...
function ww(e) {
	...
	if (n) return Tw(n);
	let r = t.dataset.documentId || t.dataset.entryId || "", i = Dw(t);
	if (!r || !i) return "";
...
function Dw(e) {
	...
	return kw(t) ? t : e.classList.contains("actor") ? "Actor" : e.classList.contains("item") ? "Item" : e.classList.contains("journal") ? "JournalEntry" : e.closest("#actors") ? "Actor" : e.closest("#items") ? "Item" : e.closest("#journal") ? "JournalEntry" : "";
}
function Ow(e, t) {
...
var Mw = {
	createDropData: Sw,
	startDocumentPick: Cw
}, Nw = class extends foundry.applications.api.ApplicationV2 {
	#e;
	getVueProps() {}
	...
	_replaceHTML(e, t, n) {
		...
		this.#e = ts(this.getVueComponent(), r), this.#e.use(xw), this.#e.provide(Kv, Mw), this.#e.mount(e);
	}
	async _preClose(e) {
...
}, Pw = "generatedSpeciesCareerTable", Fw = "Compendium.wfrp4e-core.journals.JournalEntry.wczCPcuHT4VQDLpL";
function Iw(e, t = []) {
	return Dc(e, t).flatMap((e) => [...e.emitBaseDefinition ? Rw(e.definition) : [], ...zw(e.definition, e.subspecies)]);
}
function Lw({ fallbackJournalUuid: e = Fw, flagScope: t, spec: n }) {
	return {
		displayRoll: !0,
		flags: {
			wfrp4e: {
...
function Bw(e, t) {
	return `@UUID[${e.journalUuid?.trim() || t}]{${Vw(e.name)}}`;
}
function Vw(e) {
...
function Qw(e) {
	let t = J(e) ? e : {}, n = Object.fromEntries(Zw.map((e) => [e, rT(t[e])]));
	return {
		extraSpecies: aT(t.extraSpecies),
		records: n
	};
}
function $w(e, t, n) {
	let r = Object.fromEntries(Zw.map((r) => [r, tT(r, e.records[r], t.records[r], n)]));
	return {
		extraSpecies: oT([...e.extraSpecies, ...t.extraSpecies]).filter((t) => !n.has(t) || e.extraSpecies.includes(t)),
		records: r
	};
}
function eT(e, t, n) {
...
function tT(e, t, n, r) {
	let i = e === "subspecies" ? nT(t, n) : {
		...t,
		...n
	};
	for (let e of r) Object.hasOwn(t, e) ? i[e] = iT(t[e]) : delete i[e];
	return i;
}
function nT(e, t) {
...
function rT(e) {
	return J(e) ? Object.fromEntries(Object.entries(e).map(([e, t]) => [e, iT(t)])) : {};
}
function iT(e) {
	return Array.isArray(e) ? e.map(iT) : J(e) ? Object.fromEntries(Object.entries(e).map(([e, t]) => [e, iT(t)])) : e;
}
function aT(e) {
...
function sT(e, t, n) {
	let r = lT(e, t, typeof n == "string" ? n.trim() : "");
	for (let e of r) {
...
function uT(e, t) {
	return !J(e) || !Array.isArray(e.columns) ? e : e.columns.find((e) => dT(e) === t);
}
function dT(e) {
...
function fT(e) {
	if (!J(e)) return;
	let t = hT(e), n = /@UUID\[([^\]]+)\]\{([^}]+)\}/u.exec(t), r = gT(n?.[2] ?? ""), i = gT(t) || gT(e.name), a = r || i;
	if (!a) return;
	let o = n?.[1]?.trim(), s = pT(e.range), c = mT(e.weight), l = { name: a };
...
function OT(e) {
	if (!J(e)) return;
	let t = {};
	return K(t, "die", yT(e.die)), K(t, "feet", bT(e.feet)), K(t, "inches", bT(e.inches)), Object.keys(t).length > 0 ? t : {};
}
...
function FT(e, t, n, r, i) {
	let a = {
		...
		key: t,
		name: yT(n) ?? t
	}, o = ST(eT(e, "speciesTalents", t));
	K(a, "characteristics", DT(eT(e, "speciesCharacteristics", t))), K(a, "skills", xT(eT(e, "speciesSkills", t))), K(a, "talents", o?.talents), K(a, "randomTalents", kT(TT(eT(e, "speciesRandomTalents", t)), o?.randomTalentCount)), K(a, "talentReplacements", wT(eT(e, "speciesTalentReplacement", t))), K(a, "traits", xT(eT(e, "speciesTraits", t))), IT(a, e, t), K(a, "careerTable", i.resolveCareerTable(t, void 0, void 0));
	let s = LT(e, a, i);
	return K(a, "subspecies", s.length > 0 ? s : void 0), a;
}
function IT(e, t, n) {
	K(e, "movement", bT(eT(t, "speciesMovement", n))), K(e, "fate", bT(eT(t, "speciesFate", n))), K(e, "resilience", bT(eT(t, "speciesRes", n))), K(e, "extra", bT(eT(t, "speciesExtra", n))), K(e, "age", yT(eT(t, "speciesAge", n))), K(e, "height", OT(eT(t, "speciesHeight", n))), K(e, "careerReplacements", ET(eT(t, "speciesCareerReplacements", n)));
}
function LT(e, t, n) {
	let r = eT(e, "subspecies", t.key);
	return J(r) ? Object.entries(r).flatMap(([r, i]) => r.trim() && J(i) ? [RT(e, t, r.trim(), i, n)] : []).sort(UT) : [];
}
function RT(e, t, n, r, i) {
	let a = {
		key: n,
		name: yT(r.name) ?? n
	}, o = DT(r.characteristics);
	o && K(a, "characteristics", jT(t.characteristics, o)), zT(a, t, r), VT(a, t, r), HT(a, t, r), K(a, "careerReplacements", ET(eT(e, "speciesCareerReplacements", `${t.key}-${n}`)));
	let s = wT(r.talentReplacement);
	return MT(t.talentReplacements, s) || K(a, "talentReplacements", s), K(a, "careerTable", i.resolveCareerTable(t.key, n, r.careerTable)), a;
}
function zT(e, t, n) {
	BT(e, "skills", t.skills ?? [], xT(n.skills));
	let r = ST(n.talents);
	BT(e, "talents", t.talents ?? [], r?.talents), BT(e, "traits", t.traits ?? [], xT(n.speciesTraits));
}
function BT(e, t, n, r) {
	if (!r) return;
	let i = AT(n, r);
...
function VT(e, t, n) {
	let r = ST(n.talents), i = kT(TT(n.randomTalents), r?.randomTalentCount, t.randomTalents);
	MT(t.randomTalents, i) || K(e, "randomTalents", i);
}
function HT(e, t, n) {
	NT(e, "movement", t.movement, n.movement), NT(e, "fate", t.fate, n.fate), NT(e, "resilience", t.resilience, n.resilience), NT(e, "extra", t.extra, n.extra);
}
function UT(e, t) {
...
var WT, GT = [];
function KT() {
	WT = Qw(game.wfrp4e?.config), GT = [];
}
async function qT(e) {
	GT = PT($w(WT ?? Qw(void 0), Qw(game.wfrp4e?.config), new Set(e.map((e) => e.trim()).filter(Boolean))), { resolveCareerTable: sT });
}
async function JT() {
	return structuredClone(GT);
}
//#endregion
//#region src/module/apps/species-builder/career-tables.ts
var YT = "WFRP Customizer Generated Career Tables", XT = "WFRP Customizer Career Table Link Fallback";
async function ZT(t = Yw()) {
	let n = Iw(t, await JT()), r = await QT(), i = await tE();
	await $T(r);
	for (let e of n) {
		let t = Lw({
...
async function QT() {
	let e = game.folders.contents.find((e) => e.type === "RollTable" && e.name === YT);
	if (e) return e;
...
async function $T(e) {
	let t = (game.tables?.contents ?? []).filter((t) => t.folder?.id === e.id || eE(t));
...
async function tE() {
	let e = game.journal?.contents.find((e) => e.name === XT);
	if (e) return e.uuid;
...
function iE(e, t = "Expected a Foundry Actor.") {
	if (!nE(e)) throw Error(t);
	return e;
}
function aE(e, t = "Expected a Foundry Item.") {
	if (!rE(e)) throw Error(t);
	return e;
}
function oE(e, t, n = `Expected a Foundry ${t} Item.`) {
	let r = aE(e, n);
...
async function cE(e, t) {
	...
	let r = aE(await fromUuid(n.uuid), `Drop a Foundry ${t} item here.`);
	if (r.type !== t) throw Error(`Drop a Foundry ${t} item here.`);
	let i = {
		name: t === "career" && fE(r) || r.name,
		type: t,
		uuid: r.uuid
	};
	if (t === "career") {
...
async function lE(e) {
	...
	if (!gE(n)) throw Error("Drop a Foundry Journal Entry or Journal Entry Page here.");
	return {
		name: n.name,
		uuid: n.uuid
	};
...
async function uE(e) {
	aE(await fromUuid(e), "The linked Foundry Item was not found.").sheet?.render(!0);
}
function dE(e) {
	let t = [
...
function pE(e) {
	...
	return typeof t == "string" ? /^@UUID\[([^\]]+)]{[^{}]+}$/.exec(mE(t))?.[1]?.trim() ?? "" : "";
}
function mE(e) {
	let t = e.trim();
	t = t.replaceAll(/<(p|div)>(?:\s|&nbsp;|<br\s*\/?>)*<\/\1>/gi, "").trim();
	let n = /^<(p|div)>([\S\s]*)<\/\1>$/i.exec(t);
	return n?.[2] !== void 0 && (t = n[2].trim()), hE(t.replaceAll(/<br\s*\/?>/gi, "").trim());
}
function hE(e) {
...
function bE(e, t) {
	return e.key === _E.key ? -1 : t.key === _E.key ? 1 : e.label.localeCompare(t.label);
}
//#endregion
//#region src/module/apps/species-builder/world-table/journals.ts
var xE = "generatedSpeciesJournal", SE = "WFRP Customizer Species Journals";
async function CE(e) {
	let t = game.journal?.contents ?? [], n = wE(t), r, i = [];
	for (let a of e.rows) {
		let e = TE(a.journalUuid, a.speciesKey, t) || n.get(a.speciesKey)?.uuid;
		if (!e) {
			r ??= await DE();
			let t = await JournalEntry.create({
				flags: { [$]: { [xE]: { speciesKey: a.speciesKey } } },
				folder: r.id,
				name: a.name.trim(),
				pages: []
			});
...
async function DE() {
	let e = game.folders.contents.find((e) => e.type === "JournalEntry" && e.name === SE);
	if (e) return e;
	let t = await Folder.create({
		name: SE,
		type: "JournalEntry"
...
var OE = "species", kE = "tableSettings";
async function AE(e) {
	let t = await CE(e), n = ku(t, $);
	return e.ownership === "managed" ? await PE(t, n) : await NE(t, n);
}
async function jE(e) {
	let t = game.settings.get(bw, kE);
	if (!J(t)) throw Error("WFRP table settings are unavailable; the Species table was not registered.");
	await game.settings.set(bw, kE, {
		...t,
		[OE]: e
	});
}
...
async function NE(e, t) {
	if (e.ownership === "external") {
		...
		if (!t || ME(t)) throw Error("The source Species table changed. Reload before saving a managed copy.");
	}
	if ((game.tables?.contents ?? []).some(ME)) throw Error("A managed Species table already exists. Reload before saving.");
...
async function PE(e, t) {
	...
	if (!n || !ME(n)) throw Error("The managed Species table changed. Reload before saving again.");
...
var LE = "species", RE = "tableSettings";
function zE() {
	let e = Yw(), t = new Set(e.definitions.map((e) => e.key)), n = VE().filter((e) => !t.has(e.key)), r = xu(n, e.definitions), i = game.tables?.contents ?? [], a = HE(), o = UE(i, i.filter(ME), a);
	return {
		draft: o ? WE(o, r, a[0] === o.id) : qE(),
		runtimeOptions: n
	};
}
...
function HE() {
	let e = game.settings.get(bw, RE), t = J(e) ? e[LE] : void 0;
...
function UE(e, t, n) {
	...
	return e.find((e) => e.getFlag(bw, "key") === LE);
}
function WE(e, t, n) {
	let r = e.toObject(), i = (Array.isArray(r.results) ? r.results : []).flatMap((e) => GE(e, t));
	return i.sort((e, t) => KE(e.source) - KE(t.source)), {
		isRegistered: n,
		name: e.name,
		ownership: ME(e) ? "managed" : "external",
...
function ZE(e) {
	return Array.isArray(e) ? e.filter($E) : J(e) && Array.isArray(e.contents) ? e.contents.filter($E) : eD(e) ? [...e].flatMap((e) => {
...
function tD(e, t) {
	let n = rD(e);
	if (n.reason) return dD(n.reason);
	let r = n.texts.filter((e) => !t.has(cD(e)));
	return r.length > 0 ? dD(`these result labels do not match Talent Items exactly: ${lD(r).join(", ")}`) : uD();
}
function nD(e, t) {
	let n = rD(e);
	if (n.reason) return dD(n.reason);
	let r = n.texts.filter((e) => !t.has(e));
	return r.length > 0 ? dD(`these result labels do not match tier-1 Career groups exactly: ${lD(r).join(", ")}`) : uD();
}
function rD(e) {
	if (!J(e)) return {
...
function aD(e) {
	if (!J(e)) return "";
	let t = e.type === "document" ? oD(e) : e.description;
	return typeof t == "string" ? iD(t) : "";
}
function oD(e) {
	let t = e.documentUuid, n = e.name;
	return typeof t == "string" && typeof n == "string" ? `@UUID[${t}]{${n}}` : "";
...
function fD(e, t, n, r) {
	let i = pD(e, t, typeof n == "string" ? n.trim() : "");
	for (let e of i) {
		let t = hD(e);
		if (!t) continue;
		let n = gD(t, e);
		if (n) return mD(e, n, r);
	}
	return mD(i.at(-1) ?? e, void 0, r);
}
function pD(e, t, n) {
	let r = t ? [
		n,
...
function gD(e, t) {
	return !J(e) || !Array.isArray(e.columns) ? e : e.columns.find((e) => _D(e) === t);
}
function _D(e) {
	if (!J(e) || typeof e.getFlag != "function") return "";
	let t = e.getFlag.call(e, "wfrp4e", "column");
...
function vD(e, t, n) {
	let r = bD(e.randomTalents, t.randomTalentSources, n);
	xD(e.skills, "skills", t.skills, n), SD(e.talents, r, t, n), xD(e.traits, "traits", t.traits, n), wD(e.talentReplacement, t.talents, n);
}
function yD(e, t, n) {
	if (e) {
		if (!J(e)) {
		...
		for (let [r, i] of Object.entries(e)) {
			if (t.has(r) || n.push(`Career replacement source ${AD(r)} does not match exactly`), !Array.isArray(i) || i.length === 0) {
				n.push(`Career replacements for ${AD(r)} must be a non-empty array`);
				continue;
			}
			for (let e of i) (typeof e != "string" || !t.has(e.trim())) && n.push(`Career replacement ${AD(e)} does not match exactly`);
		}
	}
}
function bD(e, t, n) {
	...
	for (let [e, i] of Object.entries(r)) {
		DD(i) || n.push(`random Talent count for ${AD(e)} is not a non-negative whole number`);
		let r = t.get(e);
		r ? r.readyForCharacterCreation || n.push(`random Talent table ${AD(e)} is not ready: ${r.reason ?? "invalid results"}`) : n.push(`random Talent table is missing for ${AD(e)}`);
	}
	return i;
}
function xD(e, t, n, r) {
	...
	let i = e.filter((e) => typeof e != "string" || !n.has(OD(e)));
	i.length > 0 && r.push(`${t} do not match available Items exactly: ${kD(i)}`);
}
function SD(e, t, n, r) {
	if (!Array.isArray(e)) {
	...
	for (let i of e) {
		if (ED(i)) {
			DD(i) || r.push(`numeric Talent grant ${AD(i)} is not a non-negative whole number`), CD("talents", t, n, r);
			continue;
		}
		if (typeof i != "string") {
			r.push(`Talent grant ${AD(i)} is not a name or numeric count`);
			continue;
		}
		let e = i.includes(",") ? i.split(",") : [i];
		for (let a of e) {
			let e = a.trim(), o = i.includes(",") ? TD(e) : void 0;
			o ? CD(o.key, t, n, r) : n.talents.has(OD(e)) || r.push(`Talent ${AD(e)} does not match an available Item exactly`);
		}
	}
}
function CD(e, t, n, r) {
	if (!t.has(e)) {
		r.push(`Talent grant uses random table ${AD(e)} without configuring that key`);
		return;
	}
	let i = n.randomTalentSources.get(e);
	i ? i.readyForCharacterCreation || r.push(`Talent grant uses unready random table ${AD(e)}`) : r.push(`Talent grant uses missing random table ${AD(e)}`);
}
function wD(e, t, n) {
	if (e) {
		if (!J(e)) {
		...
		for (let [r, i] of Object.entries(e)) t.has(OD(r)) || n.push(`Talent replacement source ${AD(r)} does not match exactly`), (typeof i != "string" || !t.has(OD(i))) && n.push(`Talent replacement ${AD(i)} does not match exactly`);
	}
}
function TD(e) {
...
function DD(e) {
	return ED(e) && Number.isInteger(Number(e)) && Number(e) >= 0;
}
function OD(e) {
	return e.split("(")[0]?.trim() ?? "";
}
function kD(e) {
	return [...new Set(e.map(AD))].join(", ");
}
function AD(e) {
...
function ND(e, t, n = []) {
	...
	return Object.entries(r).filter(([e]) => !i.has(e)).map(([n, r]) => {
		let i = typeof r == "string" && r.trim() ? r.trim() : n, a = PD(e, n, r, t);
		return a.length > 0 ? {
			key: n,
			name: i,
			readyForCharacterCreation: !1,
...
function PD(e, t, n, r) {
	...
	let a = VD(e, t);
	vD(a, r, i);
	let o = KD(e, "speciesCharacteristics", t);
	return LD(o, i), RD(UD(e, t), i), zD(e, t, i), yD(KD(e, "speciesCareerReplacements", t), r.careerGroups, i), BD(r.validateCareerTable(t, void 0, void 0), i), ID(e, t, o, a, r, i), i;
}
function FD(e) {
...
function ID(e, t, n, r, i, a) {
	...
	if (s) {
		...
		for (let [o, c] of Object.entries(s)) {
			...
			(typeof c.name != "string" || !c.name.trim()) && s.push("display name is missing"), LD(c.characteristics || n, s), RD(WD(e, t, c), s), vD(HD(c, r), i, s), yD(KD(e, "speciesCareerReplacements", `${t}-${o}`), i.careerGroups, s), BD(i.validateCareerTable(t, o, c.careerTable), s), a.push(...s.map((e) => `subspecies ${o}: ${e}`));
		}
	}
}
function LD(e, t) {
...
function zD(e, t, n) {
	let r = KD(e, "speciesAge", t);
	(typeof r != "string" || !Of(r, !1)) && n.push("age formula is missing or invalid");
	let i = KD(e, "speciesHeight", t);
	(!J(i) || typeof i.die != "string" || !Of(i.die, !0) || !qD(i.feet) || !qD(i.inches)) && n.push("height needs a valid die formula and non-negative whole feet and inches");
}
function BD(e, t) {
...
function VD(e, t) {
	return {
		randomTalents: KD(e, "speciesRandomTalents", t),
		skills: KD(e, "speciesSkills", t),
		talentReplacement: KD(e, "speciesTalentReplacement", t) || {},
		talents: KD(e, "speciesTalents", t),
		traits: KD(e, "speciesTraits", t) || []
	};
}
function HD(e, t) {
	return {
...
function UD(e, t) {
	return Object.fromEntries(MD.map(([n, r]) => [r, KD(e, n, t)]));
}
function WD(e, t, n) {
	return Object.fromEntries(MD.map(([r, i]) => [i, n[GD(r)] ?? KD(e, r, t)]));
}
function GD(e) {
...
async function XD(e = []) {
	...
	for (let e of game.items?.contents ?? []) ZD(n, e);
	for (let e of QD()) {
		if (!e.getIndex) continue;
		let t = await e.getIndex({ fields: YD });
		for (let e of ZE(t)) ZD(n, e);
	}
	let r = $D(t, n.itemNames.talent), i = new Set(n.careerGroups.keys());
	return {
		careerGroupNames: sO(n.careerGroups),
		itemBaseNames: {
			skill: sO(n.itemNames.skill),
			talent: sO(n.itemNames.talent),
			trait: sO(n.itemNames.trait)
		},
		...
		runtimeSpecies: ND(t, {
			...
			validateCareerTable: (e, t, n) => fD(e, t, n, i)
		}, e)
	};
}
function ZD(e, t) {
	...
	if (rO(t.type)) {
		aO(e.itemNames[t.type], iO(t.name));
		return;
	}
	let n = Number(Y(t, [
		"system",
	...
	typeof r == "string" && aO(e.careerGroups, r);
}
function QD() {
	let e = game.wfrp4e?.tags?.getPacksWithTag?.([...JD, "career"]);
	return e ? [...new Set(e)].filter(YE) : (game.packs ?? []).filter(YE);
}
function $D(e, t) {
	let n = [];
	for (let r of eO(e)) {
...
function eO(e) {
	...
	for (let e of game.tables?.contents ?? []) oO(t, e.getFlag("wfrp4e", "key"));
	let n = game.settings.get("wfrp4e", "tableSettings");
	J(n) && Object.keys(n).forEach((e) => t.add(e)), tO(e.speciesRandomTalents, t);
	let r = e.subspecies;
	if (J(r)) {
		for (let e of Object.values(r)) if (J(e)) for (let n of Object.values(e)) J(n) && nO(n.randomTalents, t);
	}
	return [...t];
}
function tO(e, t) {
	J(e) && Object.values(e).forEach((e) => nO(e, t));
}
function nO(e, t) {
	J(e) && Object.keys(e).forEach((e) => t.add(e));
}
function rO(e) {
	return JD.some((t) => t === e);
}
function iO(e) {
...
var cO = "WFRP Customizer Generated Species Traits";
async function lO(t = Yw()) {
	let n = ol(t, await JT()), r = await uO();
	await dO(r);
	for (let e of n) {
		let t = ll({
			flagScope: $,
...
async function uO() {
	let e = game.folders.contents.find((e) => e.type === "Item" && e.name === cO);
	if (e) return e;
	let t = await Folder.create({
		name: cO,
		type: "Item"
...
async function dO(e) {
	let t = (game.items?.contents ?? []).filter((t) => t.folder?.id === e.id || fO(t));
	for (let e of t) await e.delete();
}
function fO(e) {
	return J(Y(e.toObject(), [
...
async function pO(e, t) {
	...
	let r = (await Promise.allSettled([ZT(n), lO(n)])).flatMap((e) => e.status === "rejected" ? [hO(e.reason)] : []);
	return r.length > 0 ? {
		settings: n,
		syncError: r.join(" ")
	} : { settings: n };
...
var mO = {
	loadSettings: async () => Yw(),
	loadRandomTalentSources: async () => vE(),
	loadRuntimeSpecies: JT,
	loadValidationCatalog: XD,
	loadSpeciesTable: async () => zE(),
	openItemSheet: uE,
	resolveItemDrop: cE,
	resolveJournalDrop: lE,
	saveSettings: pO,
	saveSpeciesTable: BE
};
function hO(e) {
...
var _O = class extends Nw {
	...
	async _preClose(e) {
		let t = this.#e;
		this.#e = !1, t && await gO() && window.location.reload(), await super._preClose(e);
	}
};
//#endregion
...
function MO(e) {
	let t = /^(?<base>.+?)\s*\((?<specialization>[^)]+)\)\s*$/.exec(e.trim());
	if (!t?.groups) return null;
	let n = t.groups.base?.trim() ?? "", r = t.groups.specialization?.trim() ?? "", i = IO(r);
	return !n || !r || !FO(r, i) ? null : {
		baseName: n,
		options: i,
		originalName: e,
		specialization: r
...
function NO(e, t) {
	let n = /* @__PURE__ */ new Map();
	return t.map((t) => {
		let r = PO(t), i = n.get(r) ?? 0;
		return n.set(r, i + 1), {
			occurrence: i,
			originalName: t,
			resolutionKey: kO(e, t, i)
		};
	});
}
function PO(e) {
...
function VO(e, t) {
	...
	if (n === "custom") return HO(e.maximumFormula, t);
	let r = Number(n);
...
function UO(e, t) {
	...
	for (let r of e.careers) {
		let i = OO(KO(r, t, e.skillGrantResolutions)), a = bO(r) / 5, o = Math.max(0, EO(r.quantity) - 1) * 5;
		for (let e of i) {
			let i = yO(t, e), s = n.get(i);
			if (s) {
				a > s.highestLevel && (s.highestLevel = a, s.highestLevelSource = xO(r)), o > 0 && s.extraSources.push({
					count: o,
					kind: "career",
			...
			n.set(i, {
				extraSources: LO(r.name, o),
				highestLevel: a,
				highestLevelSource: xO(r),
				name: e
			});
		}
	}
	for (let r of n.values()) GO(e, {
		careerValue: r.highestLevel * 5 + DO(r.extraSources),
		kind: t,
		name: r.name,
		sources: [{
...
function WO(e) {
	...
	for (let n of e.careers) {
		let r = OO(KO(n, "talent", e.skillGrantResolutions)), i = Math.max(0, EO(n.quantity) - 1);
		for (let e of r) {
			let r = yO("talent", e), a = t.get(r);
			if (a) {
			...
			t.set(r, {
				extraSources: LO(n.name, i),
				firstSource: n.name,
				name: e
			});
		}
	}
	for (let n of t.values()) GO(e, {
		careerValue: 1 + DO(n.extraSources),
		kind: "talent",
		name: n.name,
		sources: [{
			count: 1,
...
function GO(e, t, n = {}) {
	let r = yO(t.kind, t.name), i = e.entries.get(r);
	if (i) {
		let r = t.kind === "talent" && i.includedFromBase ? t.sources.slice(1) : t.sources, a = t.kind === "talent" ? zO(i, DO(r), n, e.talentMaximums) : t.careerValue;
		i.careerValue = a, i.includedFromCareer = !0, i.sources = [...i.sources.filter((e) => e.kind === "base"), ...RO(r, a)];
		return;
	}
	let a = {
		baseAdvances: 0,
	...
	t.kind === "talent" && (a.careerValue = zO(a, t.careerValue, n, e.talentMaximums), a.current = a.careerValue, a.sources = RO(t.sources, a.careerValue)), e.entries.set(r, { ...a });
}
function KO(e, t, n) {
	return t === "characteristic" ? e.grants.characteristics : t === "skill" ? NO(e.uuid, e.grants.skills).map((e) => n[e.resolutionKey] || e.originalName) : e.grants.talents;
}
//#endregion
...
function qO(e, t) {
	...
	for (let r of e.values()) {
		...
		let e = kc[TO(r.name)];
		if (!e) continue;
		let i = t[yO(r.kind, r.name)] ?? 0, a = Math.max(r.minimumCurrent, Math.floor(r.careerValue + i));
		n[e] = Math.max(0, r.baseValue + a);
	}
	return n;
}
function JO(e, t, n) {
	return e.kind === "skill" ? YO(e, t, n) : e.kind === "talent" ? XO(e, t, n) : e;
}
function YO(e, t, n) {
	let r = ZO(e) ?? QO(e.name, n.skillCharacteristics) ?? $O(e.name, n.baseActorDraftData);
	if (!r) return {
		...e,
...
function XO(e, t, n) {
	let r = BO(e, n.talentMaximums), i = VO(r, t);
	return {
		...e,
...
function $O(e, t) {
	let n = t.advancements.find((t) => t.kind === "skill" && wO(t.name, e));
...
function ek(e) {
	let t = ak(e.baseActorDraftData), n = {
		careers: e.careers,
		entries: t,
		skillGrantResolutions: e.skillGrantResolutions,
		talentMaximums: e.talentMaximums
	};
	UO(n, "characteristic"), UO(n, "skill");
	let r = qO(t, e.manualAdvancementDeltas);
	return WO({
		...n,
		characteristicTotals: r
	}), ok(t, e.customAdvancements), [...t.values()].filter((t) => t.includedFromCareer || t.includedFromCustom || SO(t.kind, e.settings)).map((t) => {
		let n = JO(t, r, e), i = yO(t.kind, t.name), a = e.manualAdvancementDeltas[i] ?? 0, o = n.careerValue + a;
		return {
			...n,
...
function nk(e, t) {
	let n = Number.isFinite(t) ? t : 0;
	return tk(e, Math.max(e.minimumTotal, Math.floor(n)) - e.baseValue);
}
function rk(e, t) {
	return {
		...e,
		...Object.fromEntries(t.map((e) => [TO(e.skillName), e]))
	};
}
function ik(e, t) {
	return {
		...e,
		...Object.fromEntries(t.map((e) => [TO(e.talentName), e]))
	};
}
function ak(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of e.advancements) {
		let e = yO(n.kind, n.name), r = {
...
function fk(e) {
	...
	return typeof t == "number" && dk(e) < t;
}
function pk(e) {
	return e.filter((e) => e.kind === "talent" && fk(e)).map((e) => ({
		kind: e.kind,
		name: e.name,
...
function mk(e, t) {
	let n = new Map(e.map((e) => [gk(e), e])), r = [];
	for (let e of t) {
		let t = n.get(gk(e));
...
function yk(e, t) {
	let n = Ck(e, t), r = wk(e.skills, t.skills, _k.skill), i = Tk(e.talents, t.talents);
	return {
		characteristics: n,
		skills: r,
		talents: i,
...
function bk(e) {
	...
	return e.kind === "talent" ? Sk(t) : xk(t, e.kind === "characteristic" ? _k.characteristic : _k.skill);
}
function xk(e, t) {
...
function Ck(e, t) {
	...
	for (let r of Object.keys(Oc)) {
		let i = r, a = Dk(e.characteristics[i] ?? 0, t.characteristics[i] ?? 0);
		n += xk(a, _k.characteristic);
	}
	return n;
}
function wk(e, t, n) {
	let r = Ek(e), i = Ek(t), a = 0;
	for (let [e, t] of r) {
		let r = Dk(t, i.get(e) ?? 0);
		a += xk(r, n);
	}
	return a;
}
function Tk(e, t) {
	let n = Ek(e), r = Ek(t), i = 0;
	for (let [e, t] of n) {
		let n = Dk(t, r.get(e) ?? 0);
		i += Sk(n);
	}
	return i;
}
function Ek(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of e) {
		let e = TO(n.name), r = Math.floor(n.value);
...
function kk(e) {
	let { baseActorDraftData: t, careers: n, customAdvancements: r, manualAdvancementDeltas: i, settings: a, skillCharacteristics: o, skillGrantResolutions: s, talentMaximums: c } = e, l = W(() => ek({
	...
	})), u = W(() => vk(l.value)), d = W(() => pk(l.value).length);
	function f(e) {
		let t = yO(e.kind, e.name);
		r.value.some((e) => yO(e.kind, e.name) === t) || r.value.push(e);
	}
	function p(e) {
		let t = yO(e.kind, e.name);
		r.value = r.value.filter((e) => yO(e.kind, e.name) !== t), delete i.value[t];
	}
	function m(e, t) {
		x(e, uk(e, t));
	}
	function h() {
		for (let e of pk(l.value)) {
			let t = hk(l.value, e);
			t && x(t, e.total);
		}
	}
	function g(e, t) {
		let n = Math.max(0, Math.floor(Number.isFinite(t) ? t : 0)), r = e.run({ advancements: l.value }, n), i = mk(l.value, r.advancements);
		for (let e of i) {
			let t = hk(l.value, e);
			t && b(t, e.current);
		}
	...
	function v(e) {
		o.value = rk(o.value, e);
	}
	function y(e) {
		c.value = ik(c.value, e);
	}
	function b(e, t) {
		let n = yO(e.kind, e.name);
		i.value[n] = tk(e, t);
	}
	function x(e, t) {
		let n = yO(e.kind, e.name);
...
var Bk = new Set([
	...
	"or",
	"npc"
]), Vk = "npc-builder-portrait-filter:", Hk = "application/x-wfrp4e-customizer-portrait-filter-tag";
function Uk(e) {
	let t = [];
...
function Wk(e) {
	...
	return sA(t.flatMap((e) => oA(e)).filter((e) => e.length >= 3 && !Bk.has(e)));
}
function Gk(e) {
	return sA(oA(e).filter((e) => e.length >= 3 && !Bk.has(e)));
}
function Kk(e, t, n) {
...
function Jk(e, t) {
	let n = iA(e), r = t.mustIncludeSourceGroups.length === 0 || e.sourceGroup !== void 0 && t.mustIncludeSourceGroups.includes(e.sourceGroup), i = e.sourceGroup !== void 0 && t.mustExcludeSourceGroups.includes(e.sourceGroup);
...
function Zk(e) {
	return e.startsWith(Vk) ? e.slice(28) : null;
}
function Qk(e) {
	let t = /* @__PURE__ */ new Set(), n = [];
	for (let r of e) {
...
function $k(e) {
	let t = Qk([...Uk(e), ...e.assetCandidates]);
	return !e.selectedPortraitPath || t.some((t) => rA(t.img) === rA(e.selectedPortraitPath)) ? t : [{
...
function oA(e) {
	return aA(e).split(" ").filter(Boolean);
}
function sA(e) {
	return [...new Set(e)];
}
...
function cA() {
	...
	function r(t) {
		let r = new Set(e.value), i = Gk(t).filter((e) => !r.has(e));
...
function hA(e, t) {
	...
	return r === "petty magic" ? SA({
		kind: "petty-magic",
		rawLore: "Petty Magic",
		source: t,
		sourceName: n
	}) : r.startsWith("arcane magic") ? SA({
		kind: "arcane-magic",
		rawLore: CA(n),
		source: t,
		sourceName: n
	}) : r.startsWith("spellcaster") ? SA({
		kind: "spellcaster",
		rawLore: CA(n),
		source: t,
		sourceName: n
...
function yA(e) {
	let t = gA(e);
	return t === "petty" ? "petty" : pA.includes(t) ? "eight-wind" : mA.includes(t) ? "dark" : "other";
}
function bA(e, t) {
...
function CA(e) {
	return fA.exec(e)?.[1]?.trim() ?? "";
}
//#endregion
//#region src/functions/npc-builder/spells/derive-magic-grants.ts
function wA(e) {
	let t = /* @__PURE__ */ new Map();
	for (let n of e.advancements) n.kind !== "talent" || n.baseAdvances + n.current <= 0 || TA(t, hA(n.name, "talent"), e);
	for (let n of e.traits) TA(t, hA(n.name, "trait"), e);
	return [...t.values()];
}
function TA(e, t, n) {
	if (!t) return;
	let r = n.loreResolutions[t.resolutionKey];
	e.set(t.resolutionKey, r ? vA(t, r) : t);
}
//#endregion
...
function kA(e) {
	let t = e.detectedSpells.find((t) => AA(t, e.spell));
	return t ? {
...
function AA(e, t) {
	return e.sourceUuid && e.sourceUuid === t.sourceUuid ? !0 : wO(e.name, t.name);
}
function jA(e, t) {
...
function MA(e) {
	let { advancements: t, customSpells: n, detectedSpells: r, magicLoreResolutions: i, settings: a, spellSelectionOverrides: o, traits: s } = e, c = W(() => wA({
		advancements: t.value,
		loreResolutions: i.value,
		traits: s.value
	})), l = W(() => c.value.length > 0), u = W(() => EA({
...
function NA(e) {
	...
	if (e.allowBaseActorTraits) for (let n of e.baseActorDraftData.traits) {
		let r = LA(n);
		e.ignoredBaseTraitKeys[r] || t.set(r, BA(n, r, !1));
	}
	for (let n of e.customTraits) zA([...t.values()], n.name) || t.set(n.key, { ...n });
	return [...t.values()].map((t) => ({
		...t,
		config: CO(t.config, e.traitConfigOverrides[t.key])
	})).sort(VA);
}
function PA(e) {
	return e.allowBaseActorTraits ? [...e.baseActorDraftData.traits.filter((t) => e.ignoredBaseTraitKeys[LA(t)]).map((t) => {
		let n = LA(t);
		return {
			...BA(t, n, !0),
			config: CO(t.config, e.traitConfigOverrides[n])
		};
...
function LA(e) {
	return `base:${e.uuid || TO(e.name)}`;
}
function RA(e, t) {
	return e.find((e) => wO(e.name, t));
}
function zA(e, t) {
...
function HA(e) {
	let { baseActorDraftData: t, customTraits: n, ignoredBaseTraitKeys: r, quickTraits: i, settings: a, traitConfigOverrides: o } = e, s = W(() => NA({
	...
	function h(e) {
		zA(s.value, e.name) || n.value.some((t) => t.key === e.key) || n.value.push(e);
	}
	function g(e, t) {
		o.value[e] = {
			...o.value[e],
...
function KA(e, t) {
	let n = qA(e), r = YA(n, t), i = r.filter((e) => e.matchKind === "exact");
	return i.length === 1 ? ZA("matched", n, i[0]) : i.length > 1 ? ZA("ambiguous", n, i[0], { candidates: r }) : r.length ? {
		candidates: r,
		searchTerms: n,
		selectedCandidateUuid: "",
		selectedItemType: "trapping",
...
function qA(e) {
	...
	return t.length ? tj(t) : [e.trim()].filter(Boolean);
}
function JA(e, t) {
	if (QA(e) === QA(t)) return "exact";
	let n = $A(e), r = $A(t);
	if (!n || !r) return null;
...
function $A(e) {
	return QA(e).replaceAll("&", " and ").replaceAll(/[(),.:;[\]]/g, " ").replaceAll(/\b(a|an|the|some|pair of|pairs of)\b/g, " ").split(/\s+/).map(ej).filter(Boolean).join(" ");
}
function ej(e) {
...
function nj(e) {
	...
	return [...t.values()].map((t) => sj(t, e)).sort(cj);
}
function rj(e, t) {
...
function oj(e, t) {
	for (let n of t.careers) for (let r of n.grants.trappings) {
		...
		e.set(i, {
			...
			name: r,
			quantity: n.quantity,
			resolution: t.trappingResolutionOverrides[i] ?? GA(r),
			source: "career",
			sourceUuid: ""
		});
	}
...
function sj(e, t) {
	...
	return {
		...
		quantity: EO(n?.quantity ?? e.quantity),
...
function lj(e) {
	let { baseActorDraftData: t, careers: n, customTrappings: r, settings: i, trappingOverrides: a, trappingResolutionOverrides: o } = e, s = W(() => nj({
		baseActorDraftData: t.value,
		careers: n.value,
	...
	function f(e, t) {
		let n = s.value.find((t) => t.key === e), r = n ? rj(n, t) : null;
		r && (o.value[e] = r);
	}
	function p(e) {
...
var uj = ac("npc-builder", () => {
	let e = /* @__PURE__ */ j(""), t = /* @__PURE__ */ j([]), n = /* @__PURE__ */ j({}), r = /* @__PURE__ */ j(null), i = /* @__PURE__ */ j({ ...dA }), a = /* @__PURE__ */ j([]), o = /* @__PURE__ */ j([]), s = /* @__PURE__ */ j([]), c = /* @__PURE__ */ j([]), l = /* @__PURE__ */ j([]), u = /* @__PURE__ */ j(null), d = /* @__PURE__ */ j([]), f = /* @__PURE__ */ j([]), p = /* @__PURE__ */ j(""), m = /* @__PURE__ */ j({ ...uA }), h = /* @__PURE__ */ j(""), g = /* @__PURE__ */ j(""), _ = /* @__PURE__ */ j({}), v = /* @__PURE__ */ j({}), y = /* @__PURE__ */ j({}), b = /* @__PURE__ */ j([]), x = /* @__PURE__ */ j([]), S = /* @__PURE__ */ j([]), C = /* @__PURE__ */ j({}), ee = /* @__PURE__ */ j({}), te = /* @__PURE__ */ j({}), ne = /* @__PURE__ */ j({}), re = /* @__PURE__ */ j({}), ie = /* @__PURE__ */ j({}), w = kk({
...
}), dj = { class: "dui-fieldset-legend" }, fj = [
	"checked",
	"disabled",
	"onChange"
], pj = { class: "dui-card-actions" }, mj = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => (L(), R("section", null, [
			...
			(L(!0), R(I, null, F(e.candidateGroups, (t) => (L(), R("fieldset", {
				key: t.level,
				class: "dui-fieldset"
			}, [B("legend", dj, "Tier " + k(t.level || "Unknown"), 1), (L(!0), R(I, null, F(t.candidates, (t) => (L(), R("label", {
				key: t.uuid,
				class: "dui-label"
			}, [B("input", {
				...
				type: "checkbox",
				onChange: (e) => r(t, e)
			}, null, 40, fj), B("span", null, [B("strong", null, k(t.name), 1), B("small", null, [H(k(t.careerGroup || "Career") + " ", 1), e.isCareerQueued(t.uuid) ? (L(), R(I, { key: 0 }, [H(" already queued ")], 64)) : U("", !0)])])]))), 128))]))), 128)),
			B("div", pj, [B("button", {
				class: "dui-btn dui-btn-sm",
				type: "button",
...
}), hj = ["aria-labelledby"], gj = ["id"], _j = { class: "dui-modal-action" }, vj = /* @__PURE__ */ P({
...
function xj() {
	return [...yj.values()].sort((e, t) => e.name.localeCompare(t.name));
}
function Sj(e) {
	return yj.get(e) ?? null;
}
function Cj(e, t) {
	return Ej(e, t, {
		kinds: ["skill"],
...
function Ej(e, t, n) {
	let r = Math.max(0, Math.floor(Number.isFinite(t) ? t : 0)), i = kj(e.advancements), a = vk(i).total;
...
function Dj(e, t) {
	return t.respectTalentMaximums && e.kind === "talent" && !Oj(e) ? null : {
		...e,
		current: e.current + ck(e)
	};
}
function Oj(e) {
	let t = e.talentMaximumValue;
	return typeof t == "number" ? lk(e) < t : !1;
}
function kj(e) {
	return e.map((e) => ({
		...e,
...
function Aj(e) {
	let t = e.current - e.careerValue, n = [...e.sources].sort((e, t) => Rj(e.kind) - Rj(t.kind)).map((e) => jj(e));
	return t !== 0 && n.push(`Manual ${zj(t)}`), n.length ? n.join(", ") : e.includedFromBase ? "Base actor" : "-";
}
function jj(e) {
	return e.kind === "custom" && e.count === 0 ? e.label : `${e.label} ${zj(e.count)}`;
}
function Mj(e) {
	return MO(e) !== null;
}
...
var Bj = ["disabled"], Vj = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		}, " Reset ", 8, Bj), e.entry.includedFromCustom ? (L(), R("button", {
			key: 0,
			class: "dui-join-item dui-btn dui-btn-sm",
...
}), Hj = { class: "dui-card dui-card-border dui-card-sm" }, Uj = { class: "dui-card-body" }, Wj = { class: "dui-card-title" }, Gj = {
	key: 0,
	class: "dui-badge dui-badge-primary"
}, Kj = { key: 0 }, qj = /* @__PURE__ */ P({
	...
	setup(e) {
		return (t, n) => (L(), R("section", Hj, [B("div", Uj, [
			B("h2", Wj, [e.number ? (L(), R("span", Gj, k(e.number), 1)) : U("", !0), H(" " + k(e.title), 1)]),
			e.description ? (L(), R("p", Kj, k(e.description), 1)) : U("", !0),
			Ur(t.$slots, "default")
		])]));
	}
}), Jj = {
	...
	key: 1,
	class: "dui-alert dui-alert-info"
}, Xj = { class: "dui-list" }, Zj = { class: "dui-list-col-grow" }, Qj = {
	key: 0,
	class: "dui-badge dui-badge-info"
}, $j = {
	key: 1,
	class: "dui-badge dui-badge-warning"
}, eM = { class: "dui-join" }, tM = ["disabled", "onClick"], nM = [
	"aria-label",
	"value",
	"onInput"
], rM = ["onClick"], iM = {
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => (L(), z(qj, {
			...
			default: N(() => [
				e.manualAdvanceCount ? (L(), R("div", Jj, [B("span", null, k(e.manualAdvanceCount) + " manual edits", 1), B("button", {
					class: "dui-btn dui-btn-sm",
					type: "button",
				...
				B("ul", Xj, [(L(!0), R(I, null, F(e.entries, (t) => (L(), R("li", {
					key: `${t.kind}:${t.name}`,
					class: "dui-list-row"
				}, [B("div", Zj, [
					B("strong", null, k(t.name), 1),
					t.current === t.careerValue ? U("", !0) : (L(), R("span", Qj, " Manual edit ")),
					e.showSkillSpecializationBadges && M(Mj)(t.name) ? (L(), R("span", $j, " Needs specialization ")) : U("", !0),
					B("span", null, " Base " + k(t.baseValue) + " · Advances " + k(t.current) + " · XP " + k(M(Lj)(t)), 1),
					B("small", null, "Sources: " + k(M(Aj)(t)), 1)
				]), B("div", eM, [
					B("button", {
						class: "dui-join-item dui-btn dui-btn-sm",
						disabled: M(Nj)(t) <= t.minimumTotal,
						title: "Decrease by 5",
						type: "button",
				...
				e.entries.length ? U("", !0) : (L(), R("p", iM, "No " + k(e.title.toLowerCase()) + " to advance yet.", 1))
			]),
			_: 1
		}, 8, ["number", "title"]));
	}
}), oM = { class: "dui-fieldset" }, sM = ["value"], cM = { class: "dui-fieldset" }, lM = ["value"], uM = ["value"], dM = { key: 0 }, fM = { class: "dui-card-actions" }, pM = ["disabled"], mM = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, a) => (L(), z(qj, {
			...
			default: N(() => [
				B("fieldset", oM, [a[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Target XP", -1), B("input", {
				...
				B("fieldset", cM, [a[2] ||= B("legend", { class: "dui-fieldset-legend" }, "Strategy", -1), B("select", {
					...
					key: e.id,
					value: e.id
				}, k(e.name), 9, uM))), 128))], 40, lM)]),
				e.selectedAutoAdvanceStrategy ? (L(), R("p", dM, k(e.selectedAutoAdvanceStrategy.description), 1)) : U("", !0),
				B("div", fM, [B("button", {
					class: "dui-btn dui-btn-primary dui-btn-sm",
...
}), hM = { class: "dui-card-actions" }, gM = ["disabled"], _M = { class: "dui-list" }, vM = { class: "dui-list-col-grow" }, yM = {
	key: 0,
	class: "dui-badge dui-badge-info"
}, bM = {
	key: 1,
	class: "dui-badge dui-badge-warning"
}, xM = { class: "dui-join" }, SM = ["disabled", "onClick"], CM = [
	"aria-label",
	"value",
	"onInput"
], wM = ["onClick"], TM = {
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => (L(), z(qj, {
			...
			default: N(() => [
				B("div", hM, [B("span", null, k(e.maximizableTalentCount) + " below maximum", 1), B("button", {
					class: "dui-btn dui-btn-sm",
				...
				}, " Maximize Talents ", 8, gM)]),
				B("ul", _M, [(L(!0), R(I, null, F(e.talents, (e) => (L(), R("li", {
					key: `${e.kind}:${e.name}`,
					class: "dui-list-row"
				}, [B("div", vM, [
					B("strong", null, k(e.name), 1),
					e.current === e.careerValue ? U("", !0) : (L(), R("span", yM, " Manual edit ")),
					B("span", null, " Ranks " + k(M(Pj)(e)) + " · Maximum " + k(M(Fj)(e)) + " · XP " + k(M(Lj)(e)), 1),
					B("small", null, "Sources: " + k(M(Aj)(e)), 1),
					M(Ij)(e) ? (L(), R("span", bM, " Over maximum ")) : U("", !0)
				]), B("div", xM, [
					B("button", {
						class: "dui-join-item dui-btn dui-btn-sm",
						disabled: M(Pj)(e) <= e.minimumTotal,
						title: "Decrease by 1",
						type: "button",
				...
				e.talents.length ? U("", !0) : (L(), R("p", TM, "No talents to advance yet."))
			]),
			_: 1
		}));
	}
	...
	props: { page: {} },
	setup(e) {
		let t = uj(), { advancements: n, estimatedNpcXp: r, maximizableTalentCount: i } = oc(t), a = xj(), o = /* @__PURE__ */ j("skill-master"), s = /* @__PURE__ */ j(0), c = W(() => n.value.filter((e) => e.kind === "characteristic")), l = W(() => n.value.filter((e) => e.kind === "skill")), u = W(() => n.value.filter((e) => e.kind === "talent")), d = W(() => n.value.filter((e) => e.current !== e.careerValue).length), f = W(() => Sj(o.value) ?? a[0] ?? null), p = W(() => f.value !== null && s.value > r.value.total);
		Jn(() => r.value.total, (e) => {
			s.value < e && (s.value = e);
		}, { immediate: !0 });
		function m() {
		...
		return (n, h) => (L(), R("section", null, [e.page === "detail-characteristics" ? (L(), z(aM, {
			key: 0,
			entries: c.value,
...
var PM = { class: "dui-fieldset" }, FM = ["value"], IM = { class: "dui-fieldset" }, LM = ["disabled", "value"], RM = { value: "" }, zM = ["value"], BM = {
	key: 0,
	class: "dui-alert dui-alert-error",
	role: "alert"
}, VM = {
	...
	key: 0,
	class: "dui-avatar"
}, UM = { class: "app:size-16 app:shrink-0 app:rounded-lg" }, WM = ["src"], GM = {
	key: 1,
	class: "dui-badge"
}, KM = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [
				B("fieldset", PM, [n[0] ||= B("legend", { class: "dui-fieldset-legend" }, "Search world actors", -1), B("input", {
				...
				B("fieldset", IM, [n[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Base statblock", -1), B("select", {
					"aria-label": "Base statblock",
					class: "dui-select dui-select-sm",
					disabled: e.isLoadingActors,
					value: e.selectedBaseActorUuid,
					onChange: i
				}, [B("option", RM, k(e.isLoadingActors ? "Loading actors..." : "Choose an actor"), 1), (L(!0), R(I, null, F(e.filteredActors, (e) => (L(), R("option", {
					key: e.uuid,
					value: e.uuid
				}, k(e.name), 9, zM))), 128))], 40, LM)]),
				e.errorMessage ? (L(), R("p", BM, k(e.errorMessage), 1)) : U("", !0),
				e.selectedBaseActor ? (L(), R("article", VM, [e.selectedBaseActor.img ? (L(), R("div", HM, [B("div", UM, [B("img", {
					src: e.selectedBaseActor.img,
					alt: "",
					class: "app:h-full app:w-full app:object-cover",
					height: "64",
					width: "64"
				}, null, 8, WM)])])) : (L(), R("span", GM, k(M(AM)(e.selectedBaseActor.name)), 1)), B("div", null, [B("strong", null, k(e.selectedBaseActor.name), 1), B("span", null, [
...
}), qM = { class: "dui-card-actions" }, JM = { class: "dui-stats dui-stats-vertical app:w-full" }, YM = { class: "dui-stat" }, XM = { class: "dui-stat-value" }, ZM = {
	key: 0,
	class: "dui-stat-desc"
}, QM = { class: "dui-stat" }, $M = { class: "dui-stat-value" }, eN = {
	key: 0,
	class: "dui-stat-desc"
}, tN = {
	key: 1,
	class: "dui-stat-desc"
}, nN = { class: "dui-stat" }, rN = { class: "dui-stat-value" }, iN = { class: "dui-stat" }, aN = { class: "dui-stat-value" }, oN = { class: "dui-stat" }, sN = { class: "dui-stat-value" }, cN = { class: "dui-stat-desc" }, lN = {
	key: 0,
	class: "dui-alert dui-alert-warning",
	role: "alert"
}, uN = { key: 1 }, dN = /* @__PURE__ */ P({
	...
	setup(e) {
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [
				B("div", qM, [B("span", { class: O(["dui-badge", e.buildPreviewStatus === "Ready" ? "dui-badge-success" : "dui-badge-warning"]) }, k(e.buildPreviewStatus), 3)]),
				B("div", JM, [
					B("div", YM, [
						n[0] ||= B("span", { class: "dui-stat-title" }, "Advances", -1),
						B("strong", XM, k(e.advancementCount), 1),
						e.editedAdvanceCount ? (L(), R("small", ZM, k(e.editedAdvanceCount) + " manually edited ", 1)) : U("", !0)
					]),
					B("div", QM, [
						n[1] ||= B("span", { class: "dui-stat-title" }, "Trappings", -1),
						B("strong", $M, k(e.visibleTrappingCount), 1),
						e.fallbackTrappingCount ? (L(), R("small", eN, k(e.fallbackTrappingCount) + " blank fallback ", 1)) : U("", !0),
						e.ignoredTrappingCount ? (L(), R("small", tN, k(e.ignoredTrappingCount) + " ignored ", 1)) : U("", !0)
					]),
					B("div", nN, [n[2] ||= B("span", { class: "dui-stat-title" }, "Traits", -1), B("strong", rN, k(e.traitCount), 1)]),
					B("div", iN, [n[3] ||= B("span", { class: "dui-stat-title" }, "Spells", -1), B("strong", aN, k(e.selectedSpellCount), 1)]),
					B("div", oN, [
						n[4] ||= B("span", { class: "dui-stat-title" }, "Estimated NPC XP", -1),
						B("strong", sN, k(e.estimatedNpcXp.total), 1),
						B("small", cN, k(e.estimatedNpcXp.characteristics) + " char / " + k(e.estimatedNpcXp.skills) + " skill / " + k(e.estimatedNpcXp.talents) + " talent ", 1)
					])
				]),
				e.buildPreviewWarnings.length ? (L(), R("div", lN, [B("div", null, [(L(!0), R(I, null, F(e.buildPreviewWarnings, (e) => (L(), R("p", { key: e }, k(e), 1))), 128))])])) : (L(), R("p", uN, " The draft has a base Actor, queued Career data, resolved trappings, and a portrait ready to apply. "))
			]),
			_: 1
		}));
	}
}), fN = { class: "dui-list" }, pN = { class: "dui-list-row" }, mN = { class: "dui-list-row" }, hN = { class: "dui-list-row" }, gN = { class: "dui-list-row" }, _N = { class: "dui-list-row" }, vN = { class: "dui-list-row" }, yN = { class: "dui-list-row" }, bN = /* @__PURE__ */ P({
	...
	setup(e) {
		return (t, n) => (L(), R("dl", fN, [
			B("div", pN, [n[0] ||= B("dt", null, "Build name", -1), B("dd", null, k(e.finalActorName), 1)]),
			B("div", mN, [n[1] ||= B("dt", null, "Base actor", -1), B("dd", null, k(e.baseActorName), 1)]),
			B("div", hN, [n[2] ||= B("dt", null, "Final career", -1), B("dd", null, k(e.finalCareerName), 1)]),
			B("div", gN, [n[3] ||= B("dt", null, "Career items", -1), B("dd", null, k(e.careerItemCount), 1)]),
			B("div", _N, [n[4] ||= B("dt", null, "Apply", -1), B("dd", null, k(e.advancementCount) + " advance rows, " + k(e.visibleTrappingCount) + " trappings, " + k(e.traitCount) + " traits, " + k(e.selectedSpellCount) + " spells ", 1)]),
			B("div", vN, [n[5] ||= B("dt", null, "Extracted grants", -1), B("dd", null, k(e.grantTotals.characteristics) + " characteristics, " + k(e.grantTotals.skills) + " skills, " + k(e.grantTotals.talents) + " talents, " + k(e.grantTotals.trappings) + " trappings ", 1)]),
			B("div", yN, [n[6] ||= B("dt", null, "Estimated NPC XP", -1), B("dd", null, k(e.estimatedNpcXpTotal), 1)])
		]));
	}
}), xN = { class: "app:grid app:gap-3" }, SN = { class: "app:flex app:flex-wrap app:items-start app:gap-3" }, CN = ["aria-label", "disabled"], wN = ["src"], TN = { key: 1 }, EN = { key: 2 }, DN = { class: "app:flex app:min-w-48 app:flex-1 app:flex-col app:items-start app:gap-2" }, ON = ["title"], kN = {
	key: 1,
	...
	key: 1,
	class: "dui-fieldset"
}, PN = { class: "dui-fieldset-legend" }, FN = { key: 0 }, IN = { key: 1 }, LN = { class: "app:flex app:flex-wrap app:gap-2" }, RN = [
	"aria-label",
	"aria-pressed",
	"title",
	"onClick"
], zN = ["src"], BN = ["aria-label"], VN = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), R("section", xN, [
			B("div", SN, [B("button", {
				...
				width: "192"
			}, null, 8, wN)) : e.finalCareer ? (L(), R("strong", TN, k(M(AM)(e.finalCareer.name)), 1)) : (L(), R("span", EN, "No portrait"))], 8, CN), B("div", DN, [
				...
				}, k(e.finalPortraitPath), 9, ON)) : (L(), R("span", kN, " A Career or base Actor image will be used when available. ")),
				B("button", {
					class: "dui-btn dui-btn-outline dui-btn-sm",
					disabled: !e.portraitCandidates.length,
					type: "button",
					onClick: r[1] ||= (e) => n("openGallery")
				}, " Browse " + k(e.portraitCandidates.length) + " portraits ", 9, AN)
			])]),
			e.isLoadingPortraitCandidates && e.portraitSearchProgress ? (L(), R("div", jN, [B("progress", {
				"aria-label": "Portrait search progress",
				class: "dui-progress dui-progress-info app:w-full",
				value: e.portraitSearchProgressValue,
				max: "100"
			}, null, 8, MN), B("small", null, k(e.portraitSearchProgressLabel), 1)])) : U("", !0),
			e.portraitCandidates.length || e.isLoadingPortraitCandidates ? (L(), R("fieldset", NN, [B("legend", PN, [r[4] ||= B("span", null, "Quick picks", -1), e.isLoadingPortraitCandidates ? (L(), R("span", FN, "Updating...")) : (L(), R("span", IN, k(e.portraitCandidates.length) + " options", 1))]), B("div", LN, [(L(!0), R(I, null, F(e.compactPortraitCandidates, (t) => (L(), R("button", {
				key: t.key,
				"aria-label": M(MM)(t),
				...
				src: t.img,
				width: "64"
			}, null, 8, zN)], 10, RN))), 128)), e.hiddenPortraitCandidateCount > 0 ? (L(), R("button", {
				key: 0,
			...
			}, " +" + k(e.hiddenPortraitCandidateCount), 9, BN)) : U("", !0)])])) : U("", !0)
		]));
	}
}), HN = { class: "app:grid app:gap-3 md:app:sticky md:app:top-28 md:app:max-h-[calc(100vh-10rem)] md:app:self-start md:app:overflow-y-auto" }, UN = { class: "dui-fieldset" }, WN = ["placeholder", "value"], GN = { class: "app:hidden md:app:grid md:app:gap-3" }, KN = { class: "dui-collapse dui-collapse-arrow dui-card-border" }, qN = { class: "dui-collapse-content" }, JN = /* @__PURE__ */ P({
	__name: "BuildSidebar",
	props: {
		actorName: {},
		advancementCount: {},
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => (L(), R("aside", HN, [V(qj, {
			...
			]), B("fieldset", UN, [i[2] ||= B("legend", { class: "dui-fieldset-legend" }, "NPC name", -1), B("input", {
				"aria-label": "NPC name",
				class: "dui-input dui-input-sm",
		...
		]), B("details", KN, [i[3] ||= B("summary", { class: "dui-collapse-title" }, "Complete build details", -1), B("div", qN, [V(bN, {
			"advancement-count": e.advancementCount,
	...
	key: 0,
	class: "dui-avatar"
}, $N = { class: "app:size-10 app:rounded-md" }, eP = ["src"], tP = {
	key: 1,
	class: "dui-badge dui-badge-sm"
}, nP = { class: "dui-list-col-grow app:min-w-0" }, rP = { class: "app:flex app:min-w-0 app:flex-wrap app:items-center app:gap-1" }, iP = { class: "app:truncate" }, aP = {
	key: 0,
	class: "dui-badge dui-badge-info dui-badge-xs"
}, oP = {
	key: 1,
	class: "dui-badge dui-badge-info dui-badge-xs"
}, sP = { class: "app:flex app:min-w-0 app:items-center app:gap-2 app:text-xs" }, cP = { class: "app:shrink-0" }, lP = ["title"], uP = { class: "app:flex app:items-center app:justify-end app:gap-1" }, dP = { class: "app:flex app:items-center app:gap-1 app:text-xs" }, fP = ["value", "onInput"], pP = { class: "dui-join" }, mP = ["disabled", "onClick"], hP = ["disabled", "onClick"], gP = ["onClick"], _P = {
	key: 1,
	class: "dui-alert"
}, vP = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [e.careers.length ? (L(), R("ol", YN, [(L(!0), R(I, null, F(e.careers, (t, a) => (L(), R("li", {
				key: t.uuid,
				...
				t.img ? (L(), R("div", QN, [B("div", $N, [B("img", {
					src: t.img,
					alt: "",
					class: "app:h-full app:w-full app:object-cover",
					height: "40",
					width: "40"
				}, null, 8, eP)])])) : (L(), R("span", tP, k(M(AM)(t.name)), 1)),
				B("div", nP, [B("div", rP, [B("strong", iP, k(t.name), 1), e.draggedCareerIndex === a ? (L(), R("span", aP, " Dragging ")) : i(a) ? (L(), R("span", oP, " Place " + k(i(a)), 1)) : U("", !0)]), B("div", sP, [B("span", cP, [H(k(t.careerGroup || "Career") + " ", 1), t.level === null ? U("", !0) : (L(), R(I, { key: 0 }, [H(" level " + k(t.level), 1)], 64))]), B("small", {
					class: "dui-badge dui-badge-ghost dui-badge-sm app:min-w-0 app:truncate",
					title: [
						`Characteristics: ${M(kM)(t.grants.characteristics)}`,
						`Skills: ${M(kM)(t.grants.skills)}`,
						`Talents: ${M(kM)(t.grants.talents)}`,
						`Trappings: ${M(kM)(t.grants.trappings)}`
					].join("\n")
				}, k(M(OM)(t)), 9, lP)])]),
				B("div", uP, [B("label", dP, [n[1] ||= H(" Qty ", -1), B("input", {
			...
			], 42, XN))), 128))])) : (L(), R("p", _P, "No careers queued yet."))]),
			_: 1
		}));
	}
}), yP = { class: "app:grid app:gap-3" }, bP = { class: "dui-fieldset" }, xP = ["for"], SP = { class: "dui-join app:w-full" }, CP = ["id", "onKeydown"], wP = { class: "app:grid app:gap-2 md:app:grid-cols-3" }, TP = [
	"onDragenter",
	"onDragleave",
	"onDragover",
	"onDrop"
], EP = { class: "dui-fieldset-legend app:flex app:items-center app:gap-2" }, DP = { class: "app:flex app:flex-wrap app:gap-2" }, OP = [
	"title",
	"onClick",
	"onDragstart",
	"onKeydown"
], kP = { "aria-live": "polite" }, AP = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		function d(e, t) {
			t.stopPropagation(), o.value = e, t.dataTransfer?.setData("text/plain", Xk(e.id)), t.dataTransfer?.setData(Hk, e.id), t.dataTransfer && (t.dataTransfer.effectAllowed = "move");
		}
		function f(e, t) {
		...
		function m(e, t) {
			...
			let i = t.dataTransfer?.getData("application/x-wfrp4e-customizer-portrait-filter-tag") || Zk(t.dataTransfer?.getData("text/plain") ?? ""), a = o.value ?? n.tags.find((e) => e.id === i) ?? null;
		...
		return (e, t) => (L(), R("section", yP, [
			B("fieldset", bP, [
				...
				}, null, 40, CP), [[Uo, i.value]]), B("button", {
					class: "dui-btn dui-btn-sm dui-join-item",
					type: "button",
					onClick: u
				}, " Add ")])
			...
			B("div", wP, [(L(), R(I, null, F(c, (e) => B("fieldset", {
				key: e.id,
				...
				onDrop: (t) => m(e.id, t)
			}, [B("legend", EP, [B("span", null, k(e.title), 1), B("span", {
				"aria-live": "polite",
				class: O(["dui-badge dui-badge-info dui-badge-sm", { "app:invisible": s.value !== e.id }])
			}, k(y(e.id) || "Drop here"), 3)]), B("div", DP, [(L(!0), R(I, null, F(l.value[e.id], (e) => (L(), R("button", {
				key: e.id,
			...
			B("div", {
				...
				"aria-hidden": "true",
				class: "fa-solid fa-trash"
			}, null, -1), B("span", kP, k(b.value), 1)], 34)
		]));
	}
}), jP = ["aria-busy"], MP = { class: "app:flex app:flex-wrap app:items-start app:justify-between app:gap-2" }, NP = { class: "dui-badge dui-badge-outline" }, PP = {
	key: 0,
	"aria-live": "polite",
	class: "dui-alert dui-alert-info",
	role: "status"
}, FP = { class: "app:flex-1" }, IP = ["value"], LP = {
	key: 1,
	class: "dui-alert dui-alert-warning"
}, RP = {
	key: 2,
	class: "app:max-h-[26rem] app:overflow-y-auto app:pr-1"
}, zP = { class: "dui-list app:m-0 app:grid app:grid-cols-[repeat(auto-fill,minmax(8.5rem,1fr))] app:gap-3 app:p-0" }, BP = [
	"aria-label",
	"aria-pressed",
	"title",
	"onClick"
], VP = ["src"], HP = { class: "app:flex app:flex-wrap app:items-center app:justify-between app:gap-1" }, UP = {
	key: 0,
	class: "dui-badge dui-badge-success dui-badge-sm"
}, WP = { class: "app:text-sm" }, GP = {
	key: 3,
	class: "dui-alert"
}, KP = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), z(vj, {
			...
			default: N(() => [B("section", {
				...
				B("header", MP, [r[3] ||= B("div", null, [B("p", { class: "app:m-0" }, "Select a portrait to apply it immediately to this draft."), B("small", { class: "app:text-base-content/70" }, " Career and base Actor images remain available regardless of search terms. ")], -1), B("span", NP, k(e.portraitCandidates.length) + " options ", 1)]),
				V(AP, {
				...
				e.isLoadingPortraitCandidates ? (L(), R("div", PP, [B("div", FP, [
				...
				])])) : e.portraitSearchTerms.length && !e.portraitCandidates.length ? (L(), R("p", LP, " No portraits match the current filter tags. ")) : U("", !0),
				e.portraitCandidates.length ? (L(), R("div", RP, [B("ul", zP, [(L(!0), R(I, null, F(e.portraitCandidates, (t) => (L(), R("li", { key: t.key }, [B("button", {
					"aria-label": M(MM)(t),
					"aria-pressed": t.key === e.selectedPortraitCandidateKey,
					...
					B("span", HP, [B("small", null, k(M(NM)(t)), 1), t.key === e.selectedPortraitCandidateKey ? (L(), R("span", UP, " Selected ")) : U("", !0)]),
					B("strong", WP, k(t.label), 1)
				], 10, BP)]))), 128))])])) : e.isLoadingPortraitCandidates ? U("", !0) : (L(), R("p", GP, " No portraits are available yet. Choose a base Actor or queue a Career to start the search. "))
			], 8, jP)]),
			_: 1
		}, 8, ["open"]));
	}
	...
	key: 0,
	class: "dui-avatar"
}, YP = { class: "app:size-14 app:shrink-0 app:rounded-lg" }, XP = ["src"], ZP = {
	key: 1,
	class: "dui-badge"
}, QP = {
	key: 1,
	class: "dui-alert dui-alert-info"
}, $P = { class: "dui-card-actions" }, eF = ["disabled"], tF = {
	key: 2,
	class: "dui-alert"
}, nF = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), z(qj, {
			...
			default: N(() => [
				e.finalCareer ? (L(), R("article", qP, [e.finalCareer.img ? (L(), R("div", JP, [B("div", YP, [B("img", {
					src: e.finalCareer.img,
					alt: "",
					class: "app:h-full app:w-full app:object-cover",
					height: "56",
					width: "56"
				}, null, 8, XP)])])) : (L(), R("span", ZP, k(M(AM)(e.finalCareer.name)), 1)), B("div", null, [
					...
					B("small", null, k(M(OM)(e.finalCareer)), 1)
				])])) : U("", !0),
				e.careers.length > 1 ? (L(), R("div", QP, [B("span", null, k(e.careers.length - 1) + " lower-tier Career" + k(e.careers.length === 2 ? "" : "s"), 1), B("span", null, "Included before " + k(e.finalCareer?.name) + ".", 1)])) : U("", !0),
				B("div", $P, [B("button", {
					class: "dui-btn dui-btn-sm",
				...
				e.careers.length ? U("", !0) : (L(), R("p", tF, "No Career selected."))
			]),
			_: 1
		}));
	}
	...
	key: 0,
	class: "dui-fieldset"
}, iF = { class: "dui-fieldset-legend" }, aF = { class: "dui-card-actions" }, oF = ["aria-pressed", "onClick"], sF = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		let n = t;
		return (t, r) => e.traits.length ? (L(), R("fieldset", rF, [B("legend", iF, [B("span", null, k(e.title), 1), B("span", null, k(e.caption), 1)]), B("div", aF, [(L(!0), R(I, null, F(e.traits, (e) => (L(), R("button", {
			key: e.uuid,
			"aria-pressed": e.isSelected,
			class: O(["dui-btn dui-btn-sm", { "dui-btn-active": e.isSelected }]),
			type: "button",
			onClick: (t) => n("toggleTrait", e)
		}, k(e.name), 11, oF))), 128))])])) : U("", !0);
	}
});
//#endregion
...
function lF(e, t) {
	let n = uj(), { baseActors: r, selectedBaseActorUuid: i } = oc(n), a = /* @__PURE__ */ j(""), o = W(() => {
...
function dF() {
	let e = uj(), { optionalTraits: t, quickTraits: n, traits: r } = oc(e), i = W(() => new Set(r.value.map((e) => fF(e.name)))), a = W(() => t.value.map(s)), o = W(() => {
		let e = new Set(t.value.map((e) => fF(e.name)));
		return n.value.filter((t) => !e.has(fF(t.name))).map(s);
	});
	function s(e) {
		return {
			...e,
			isSelected: i.value.has(fF(e.name))
		};
	}
	function c(t) {
		let n = i.value.has(fF(t.name));
...
function pF() {
	let e = uj(), t = /* @__PURE__ */ j(null), n = /* @__PURE__ */ j(null);
	function r(t, n) {
		let r = n.target;
...
function mF(e, t) {
	let n = uj(), { careers: r, customPortraitSearchTerms: i, finalPortraitPath: a, portraitSourceTagSections: o, portraitTermSections: s, selectedBaseActor: c, selectedPortraitPath: l, settings: u } = oc(n), d = /* @__PURE__ */ j([]), f = /* @__PURE__ */ j(!1), p = /* @__PURE__ */ j(null), m = 0, h = W(() => $k({
		...
		careers: r.value,
		selectedBaseActor: c.value
	})), _ = W(() => hF([...g.value, ...i.value])), v = W(() => Kk(_.value, s.value, "search")), y = W(() => Kk(_.value, s.value, "must-include")), b = W(() => Kk(_.value, s.value, "must-exclude")), x = W(() => hF([...v.value, ...y.value])), S = W(() => hF(h.value.map((e) => e.sourceGroup).filter((e) => !!e))), C = W(() => [..._.value.flatMap(le), ...S.value.map(ue)]), ee = W(() => de("must-include")), te = W(() => de("must-exclude")), ne = W(() => h.value.filter((e) => Jk(e, {
		mustExcludeSourceGroups: te.value,
		mustExcludeTerms: b.value,
		mustIncludeSourceGroups: ee.value,
		mustIncludeTerms: y.value
	}))), re = W(() => ne.value.slice(0, 4)), ie = W(() => Math.max(0, ne.value.length - re.value.length)), w = W(() => ne.value.find((e) => e.img === a.value) ?? null), T = W(() => w.value?.key ?? ""), ae = W(() => nA(p.value)), oe = W(() => tA(p.value));
	Jn(_, (e) => {
...
var vF = { class: "app:grid app:gap-3" }, yF = { class: "app:grid app:items-start app:gap-3 md:app:grid-cols-[minmax(0,1fr)_minmax(17rem,22rem)]" }, bF = { class: "app:grid app:min-w-0 app:gap-3" }, xF = /* @__PURE__ */ P({
	...
	setup(e) {
		let t = e, n = uj(), { actorName: r, advancements: i, careers: a, estimatedNpcXp: o, finalActorName: s, finalCareer: c, finalPortraitPath: l, grantTotals: u, selectedBaseActor: d, selectedSpells: f, suggestedActorName: p, traits: m } = oc(n), h = /* @__PURE__ */ j(""), { actorFilter: g, filteredActors: _, selectedBaseActorSelectValue: v } = lF(t.bridge, h), { clearCareerDragState: y, draggedCareerIndex: b, dragOverCareerIndex: x, handleCareerDragOver: S, handleCareerDragStart: C, handleCareerDrop: ee, moveCareer: te, removeCareer: ne, setCareerQuantity: re, setDragOverCareerIndex: ie } = pF(), { displayedQuickTraitOptions: w, optionalTraitOptions: T, toggleOptionalTrait: ae, toggleQuickTrait: oe } = dF(), { buildPreviewStatus: E, buildPreviewWarnings: se, careerItemCount: D, editedAdvanceCount: ce, fallbackTrappingCount: le, ignoredTrappingCount: ue, visibleTrappingCount: de } = uF(), { addPortraitSearchTerm: fe, compactPortraitCandidates: pe, hiddenPortraitCandidateCount: me, isLoadingPortraitCandidates: he, isPortraitGalleryOpen: ge, portraitCandidates: _e, portraitFilterTags: O, portraitSearchProgress: ve, portraitSearchProgressLabel: ye, portraitSearchProgressValue: be, portraitSearchTerms: xe, selectedPortraitCandidate: Se, selectedPortraitCandidateKey: Ce, selectPortrait: we, selectPortraitFromGallery: k, setPortraitFilterTagSection: Te } = _F(t.bridge, h);
		return (t, Ee) => (L(), R("section", vF, [B("div", yF, [B("div", bF, [
			e.page === "build-quick" || e.page === "build-actor" ? (L(), z(KM, {
				key: 0,
				"actor-filter": M(g),
	...
	Small: "sml",
	Tiny: "tiny"
}, CF = new Set(["bestial", "skittish"]);
function wF(e) {
	let t = e.some((e) => kF(e, "skittish")), n = e.some((e) => AF(e, "trained", "war")), r = MF(e.filter((e) => kF(e, "weapon")));
	return e.map((e) => {
		let i = EF(e.name);
		return CF.has(i) ? OF(e, `${e.name} is removed from combined mounts.`) : i === "weapon" ? !n || t ? OF(e, "Weapon requires Trained (War) and a mount that was not Skittish.") : e.uuid === r ? DF(e, "Weapon (Mount)") : OF(e, "Only the strongest Weapon trait is retained for the combined profile.") : DF(e, e.damage ? jF(e.name) : e.name);
	});
}
function TF(e) {
	return EF(e) === "armour";
...
function AF(e, t, n) {
	return kF(e, t) ? e.specification.trim().toLocaleLowerCase() === n : e.name.trim().toLocaleLowerCase() === `${t} (${n})`;
}
function jF(e) {
...
var NF = [
	...
	[SF.Average]: "Average",
	[SF.Enormous]: "Enormous",
	[SF.Large]: "Large",
	[SF.Little]: "Little",
	[SF.Monstrous]: "Monstrous",
	[SF.Small]: "Small",
	[SF.Tiny]: "Tiny"
};
function FF(e, t) {
...
function zF({ flagScope: e, mount: t, plan: n, rider: r }) {
...
function BF(e, t) {
	...
	return t.chargeStrengthBonus > 0 && n.push({
		...
		script: VF(e, t.chargeStrengthBonus),
		trigger: "preRollTest"
	}), n;
}
function VF(e, t) {
...
function HF(e, t, n) {
	return [
		...
		`<p><strong>Rider:</strong> ${UF(e.name)}<br><strong>Mount:</strong> ${UF(t.name)}</p>`,
...
var GF = { class: "app:max-w-full app:overflow-x-auto" }, KF = { class: "dui-table dui-table-sm" }, qF = { class: "dui-alert" }, JF = { class: "app:flex app:flex-wrap app:gap-2" }, YF = { key: 0 }, XF = {
	key: 1,
	...
	setup(e) {
		let t = e, n = W(() => t.plan.traits.filter((e) => e.included)), r = W(() => t.plan.traits.filter((e) => !e.included)), i = W(() => [
			...
			{
				field: "Size",
				mount: PF[t.mount.size],
				result: PF[t.plan.size],
				rider: PF[t.rider.size],
				rule: "Larger"
			}
		]);
		function a(e) {
		...
		return (t, o) => (L(), R(I, null, [V(qj, {
			...
			default: N(() => [B("div", GF, [B("table", KF, [o[0] ||= B("thead", null, [B("tr", null, [
				B("th", null, "Field"),
				B("th", null, "Rider"),
				B("th", null, "Mount"),
				B("th", null, "Combined"),
			...
			]))), 128))])])]), B("p", qF, " Charge attacks gain +" + k(e.plan.chargeStrengthBonus) + " Damage from the mount's Strength Bonus. The combined profile also gains at least Armour (1). ", 1)]),
			_: 1
		}), V(qj, {
			...
			default: N(() => [
				B("div", JF, [(L(!0), R(I, null, F(n.value, (e) => (L(), R("span", {
					key: e.sourceUuid,
					class: "dui-badge dui-badge-sm"
				}, k(a(e)), 1))), 128))]),
				n.value.length ? U("", !0) : (L(), R("p", YF, "The mount contributes no traits.")),
				r.value.length ? (L(), R("div", XF, [o[1] ||= B("p", null, [B("strong", null, "Removed or consolidated")], -1), (L(!0), R(I, null, F(r.value, (e) => (L(), R("p", {
					key: e.sourceUuid,
					class: "dui-alert dui-alert-warning"
...
}), QF = { class: "app:grid app:gap-3" }, $F = { class: "app:grid app:gap-3 md:app:grid-cols-2" }, eI = { class: "dui-fieldset" }, tI = ["for"], nI = ["id"], rI = { class: "dui-fieldset" }, iI = ["for"], aI = [
	"id",
	"disabled",
	"value"
], oI = ["value"], sI = {
	...
	key: 0,
	class: "dui-avatar"
}, pI = { class: "app:size-16 app:shrink-0 app:rounded-lg" }, mI = ["src"], hI = /* @__PURE__ */ P({
	__name: "NpcBuilderMountTab",
	props: { bridge: {} },
	setup(e) {
		let t = e, n = uj(), { baseActorCombatProfile: r, mountActorProfile: i, mountActors: a, selectedBaseActorUuid: o, selectedMountActorUuid: s } = oc(n), c = /* @__PURE__ */ j(""), l = /* @__PURE__ */ j(""), u = /* @__PURE__ */ j(!1), d = mr(), f = 0, p = W(() => {
			let e = c.value.trim().toLocaleLowerCase();
			return a.value.filter((t) => t.uuid !== o.value && (!e || t.name.toLocaleLowerCase().includes(e)));
		}), m = W(() => a.value.find((e) => e.uuid === s.value) ?? null), h = W(() => !r.value || !i.value ? null : FF(r.value, i.value));
		Jn(s, async (e) => {
			let r = ++f;
			if (l.value = "", !e) {
			...
			try {
				...
				r === f && (n.hydrateMountActorProfile(null), l.value = WF(e));
			} finally {
				r === f && (u.value = !1);
			}
		}, { immediate: !0 });
		...
		return (e, t) => (L(), R("div", QF, [
			...
			V(qj, {
				...
				default: N(() => [
					...
					M(s) ? (L(), R("div", sI, [B("button", {
						class: "dui-btn dui-btn-ghost dui-btn-sm",
						type: "button",
						onClick: t[1] ||= (...e) => M(n).clearMountSelection && M(n).clearMountSelection(...e)
					}, " Clear Mount ")])) : U("", !0),
					M(o) ? l.value ? (L(), R("p", lI, k(l.value), 1)) : u.value ? (L(), R("p", uI, " Loading mount profile... ")) : m.value && M(i) ? (L(), R("article", dI, [m.value.img ? (L(), R("div", fI, [B("div", pI, [B("img", {
						src: m.value.img,
						alt: "",
						class: "app:h-full app:w-full app:object-cover",
						height: "64",
						width: "64"
					}, null, 8, mI)])])) : U("", !0), B("div", null, [B("strong", null, k(m.value.name), 1), B("span", null, " Movement " + k(M(i).movement) + " | Wounds " + k(M(i).wounds) + " | " + k(M(PF)[M(i).size]), 1)])])) : U("", !0) : (L(), R("p", cI, " Choose the rider on the Build tab before selecting a mount. "))
				]),
				_: 1
			}),
			h.value && M(r) && M(i) ? (L(), z(ZF, {
				key: 0,
				mount: M(i),
				plan: h.value,
				rider: M(r)
...
function vI(e) {
	let t = uj(), { actorFolders: n, itemFolders: r, settings: i } = oc(t), a = /* @__PURE__ */ j(""), o = /* @__PURE__ */ j(""), s = /* @__PURE__ */ j(!1), c = /* @__PURE__ */ j(""), l = /* @__PURE__ */ j(null), u = /* @__PURE__ */ j(""), d = /* @__PURE__ */ j(""), f = W(() => l.value?.digDownActive ?? !0), p = W(() => gI(l.value));
	Jn(l, (e) => {
	...
	async function C() {
		await ee(async () => {
			t.hydrateSettings(await e.saveSettings(lA())), await te(), d.value = "Settings reset to defaults.";
		});
	}
	async function ee(e) {
...
var bI = { class: "dui-fieldset" }, xI = { class: "dui-fieldset-legend" }, SI = ["aria-label", "value"], CI = { value: "" }, wI = ["value"], TI = { class: "dui-fieldset" }, EI = ["aria-label", "value"], DI = { class: "dui-card-actions" }, OI = ["disabled"], kI = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, a) => (L(), R("section", null, [
			B("fieldset", bI, [B("legend", xI, k(e.folderLabel), 1), B("select", {
				"aria-label": e.folderLabel,
				class: "dui-select dui-select-sm",
				value: e.selectedUuid,
				onChange: r
			}, [B("option", CI, k(e.defaultOptionLabel), 1), (L(!0), R(I, null, F(e.folders, (e) => (L(), R("option", {
				key: e.uuid,
				value: e.uuid
			}, k(e.name), 9, wI))), 128))], 40, SI)]),
			B("fieldset", TI, [a[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Create or use by name", -1), B("input", {
			...
			}, k(e.buttonLabel ?? "Save Folder"), 9, OI)])
		]));
	}
}), AI = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => (L(), z(qj, {
			...
			default: N(() => [
				e.showAdvancementFeatures === !1 ? U("", !0) : (L(), R("label", jI, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.allowCharacteristics,
					type: "checkbox",
					onChange: i[0] ||= (e) => n("allowCharacteristicsChange", r(e))
				}, null, 40, MI), i[5] ||= B("span", null, "Show base actor characteristics", -1)])),
				e.showAdvancementFeatures === !1 ? U("", !0) : (L(), R("label", NI, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.allowSkills,
					type: "checkbox",
					onChange: i[1] ||= (e) => n("allowSkillsChange", r(e))
				}, null, 40, PI), i[6] ||= B("span", null, "Show base actor skills", -1)])),
				e.showAdvancementFeatures === !1 ? U("", !0) : (L(), R("label", FI, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.allowTalents,
					type: "checkbox",
					onChange: i[2] ||= (e) => n("allowTalentsChange", r(e))
				}, null, 40, II), i[7] ||= B("span", null, "Show base actor talents", -1)])),
				e.showTrappingFeature ? (L(), R("label", LI, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.allowTrappings,
					type: "checkbox",
					onChange: i[3] ||= (e) => n("allowTrappingsChange", r(e))
				}, null, 40, RI), i[8] ||= B("span", null, "Show base actor trappings", -1)])) : U("", !0),
				e.showTraitFeature === !1 ? U("", !0) : (L(), R("label", zI, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.allowTraits,
					type: "checkbox",
					onChange: i[4] ||= (e) => n("allowTraitsChange", r(e))
				}, null, 40, BI), i[9] ||= B("span", null, "Show base actor traits", -1)]))
			]),
			_: 1
		}));
	}
}), HI = { class: "dui-label" }, UI = ["checked"], WI = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [B("label", HI, [B("input", {
				class: "dui-toggle dui-toggle-sm",
				checked: e.autoSelectGrantedSpells,
				type: "checkbox",
				onChange: r
			}, null, 40, UI), n[0] ||= B("span", null, "Select detected Lore spells by default", -1)])]),
			_: 1
		}));
	}
}), GI = { class: "dui-label" }, KI = ["checked"], qI = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [B("label", GI, [B("input", {
				class: "dui-toggle dui-toggle-sm",
				checked: e.includeSpeciesInName,
				type: "checkbox",
				onChange: r
			}, null, 40, KI), n[0] ||= B("span", null, "Include species in suggested names", -1)])]),
			_: 1
		}));
	}
}), JI = { class: "dui-fieldset" }, YI = ["value"], XI = { class: "dui-label" }, ZI = ["checked"], QI = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), z(qj, { title: "Career Resolution" }, {
			default: N(() => [B("fieldset", JI, [n[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Lower career handling", -1), B("select", {
			...
			]], 40, YI)]), B("label", XI, [B("input", {
				class: "dui-toggle dui-toggle-sm",
				checked: e.askForLinkedSkillSpecializations,
				type: "checkbox",
				onChange: i
			}, null, 40, ZI), n[2] ||= B("span", null, "Resolve linked career skill repeats separately", -1)])]),
			_: 1
		}));
	}
}), $I = { class: "dui-label" }, eL = ["checked", "disabled"], tL = {
	"aria-live": "polite",
	class: "dui-alert",
	role: "status"
}, nL = { class: "dui-label" }, rL = ["checked"], iL = { class: "dui-label" }, aL = ["checked"], oL = /* @__PURE__ */ P({
		...
		return (t, n) => (L(), z(qj, {
			...
			default: N(() => [
				B("label", $I, [B("input", {
					...
					type: "checkbox",
					onChange: r
				}, null, 40, eL), n[0] ||= B("span", null, "Search Dig Down's file cache for portrait suggestions", -1)]),
				B("p", tL, k(e.statusLabel), 1),
				B("label", nL, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.searchCompendiumPortraitAssets,
					type: "checkbox",
					onChange: i
				}, null, 40, rL), n[1] ||= B("span", null, "Search Actor and Item compendiums for portrait suggestions", -1)]),
				B("label", iL, [B("input", {
					class: "dui-toggle dui-toggle-sm",
					checked: e.searchWebPortraitAssets,
					disabled: "",
					type: "checkbox"
				}, null, 8, aL), n[2] ||= B("span", null, "Search the web for portrait suggestions (later)", -1)])
			]),
			_: 1
		}));
	}
}), sL = { class: "dui-card-actions" }, cL = ["disabled"], lL = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), z(qj, {
			...
			}, " Import Recommended Quick Traits ", 8, cL)])]),
			_: 1
		}));
	}
}), uL = {
	...
	setup(e) {
		return (t, n) => e.errorMessage ? (L(), R("p", uL, k(e.errorMessage), 1)) : e.settingsMessage ? (L(), R("p", dL, k(e.settingsMessage), 1)) : U("", !0);
	}
}), pL = { class: "app:grid app:gap-3" }, mL = { class: "app:grid app:grid-cols-[repeat(auto-fit,minmax(18rem,1fr))] app:gap-3" }, hL = { class: "dui-card-actions" }, gL = ["disabled"], _L = ["disabled"], vL = /* @__PURE__ */ P({
	...
	setup(e) {
		...
		return kr(() => {
			...
			B("div", mL, [
				e.page === "settings-folders" ? (L(), z(AI, {
					key: 0,
				...
				e.page === "settings-folders" ? (L(), z(lL, {
					key: 1,
					"is-busy": M(o),
					"item-folders": M(s),
				...
				e.page === "settings-suggestions" ? (L(), z(qI, {
					key: 2,
				...
				e.page === "settings-suggestions" ? (L(), z(oL, {
					key: 3,
				...
				e.page === "settings-advancement" ? (L(), z(VI, {
					key: 4,
				...
				e.page === "settings-resolution" ? (L(), z(WI, {
					key: 5,
				...
				e.page === "settings-resolution" ? (L(), z(VI, {
					key: 6,
					"allow-characteristics": M(_).allowBaseActorCharacteristics,
					"allow-skills": M(_).allowBaseActorSkills,
					"allow-talents": M(_).allowBaseActorTalents,
				...
				e.page === "settings-resolution" ? (L(), z(QI, {
					key: 7,
					class: "app:col-span-full",
			...
			}, " Save Settings ", 8, gL), B("button", {
				class: "dui-btn dui-btn-sm",
				disabled: M(o),
				type: "button",
				onClick: y[23] ||= (...e) => M(f) && M(f)(...e)
			}, " Reset to Defaults ", 8, _L)])
		]));
	}
});
//#endregion
...
function TL(e) {
	let t = uj(), { magicGrants: n, spells: r, selectedSpells: i } = oc(t), a = /* @__PURE__ */ j(""), o = /* @__PURE__ */ j(!1), s = /* @__PURE__ */ j(!1), c = /* @__PURE__ */ j([]), l = /* @__PURE__ */ j(null), u = 0, d = W(() => bL(n.value)), f = W(() => n.value.length - d.value.length);
	Jn(() => yL(n.value), () => {
		m();
	});
	function p() {
	...
	async function g() {
		a.value = "", await h(), l.value = xL(n.value, c.value);
	}
	function _() {
		let e = l.value;
		if (e) {
...
var DL = { class: "dui-card-body" }, OL = { class: "dui-card-title" }, kL = { class: "dui-fieldset" }, AL = ["onUpdate:modelValue", "aria-label"], jL = ["value"], ML = { class: "dui-card-actions" }, NL = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), R("section", null, [
			...
			(L(!0), R(I, null, F(e.prompt.rows, (e) => (L(), R("section", {
				...
				B("h3", OL, k(e.grantLabel), 1),
				B("span", null, k(e.sourceLabel) + " - " + k(e.rawLore || "Any Lore"), 1),
				B("fieldset", kL, [r[3] ||= B("legend", { class: "dui-fieldset-legend" }, "Lore", -1), Vn(B("select", {
					...
					key: e.key,
					value: e.value
				}, k(e.label) + k(e.wind && e.wind !== "None" ? ` (${e.wind})` : ""), 9, jL))), 128))], 8, AL), [[Wo, e.selectedLore]])])
			])]))), 128)),
			B("div", ML, [B("button", {
				class: "dui-btn dui-btn-sm",
				type: "button",
	...
	key: 1,
	class: "dui-list"
}, IL = { class: "dui-list-col-grow" }, LL = { key: 0 }, RL = { key: 1 }, zL = {
	key: 2,
	class: "dui-card-actions"
}, BL = ["disabled"], VL = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), z(qj, {
			...
			default: N(() => [e.magicGrants.length ? (L(), R("ul", FL, [(L(!0), R(I, null, F(e.magicGrants, (e) => (L(), R("li", {
				key: `${e.source}:${e.sourceName}:${e.rawLore}`,
				class: "dui-list-row"
			}, [B("div", IL, [
				B("strong", null, k(M(SL)(e)), 1),
				B("span", null, k(M(wL)(e)) + " - " + k(e.sourceName), 1),
				e.isAmbiguous ? (L(), R("small", LL, " Needs Lore resolution before automatic spells can be found. ")) : (L(), R("small", RL, " Lore: " + k(e.rawLore || e.normalizedLore), 1))
			])]))), 128))])) : (L(), R("p", PL, " No magic-enabling Talent or Trait is selected. ")), e.ambiguousGrantCount ? (L(), R("div", zL, [B("button", {
				class: "dui-btn dui-btn-sm",
				disabled: e.isLoadingLoreOptions,
				type: "button",
				onClick: r[0] ||= (e) => n("resolveLores")
			}, k(e.isLoadingLoreOptions ? "Loading Lores..." : "Resolve Lores"), 9, BL)])) : U("", !0)]),
			_: 1
		}));
	}
});
...
var UL = { class: "dui-card-actions" }, WL = ["disabled"], GL = {
	key: 0,
	class: "dui-alert dui-alert-error",
	role: "alert"
}, KL = {
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), z(qj, {
			...
			default: N(() => [
				...
				}, k(e.isLoadingSpells ? "Finding spells..." : "Refresh Spells"), 9, WL), B("span", null, k(e.selectedSpellCount) + " selected / " + k(e.spells.length) + " found", 1)]),
				e.errorMessage ? (L(), R("p", GL, k(e.errorMessage), 1)) : U("", !0),
				e.ambiguousGrantCount ? (L(), R("p", KL, k(e.ambiguousGrantCount) + " magic grant" + k(e.ambiguousGrantCount === 1 ? "" : "s") + " still need Lore resolution. You can still drop specific spells for now. ", 1)) : U("", !0),
				e.spells.length ? (L(), R("ul", qL, [(L(!0), R(I, null, F(e.spells, (e) => (L(), R("li", {
					key: e.key,
					class: "dui-list-row"
				}, [
					B("input", {
					...
					B("div", YL, [
						e.img ? (L(), R("div", XL, [B("div", null, [B("img", {
							src: e.img,
							alt: ""
						}, null, 8, ZL)])])) : U("", !0),
						B("strong", null, k(e.name), 1),
						B("span", null, k(e.loreName || "Unknown Lore") + " · " + k(M(HL)(e)), 1)
					]),
				...
				]))), 128))])) : (L(), R("p", $L, " No matching spells found yet. Drop specific spells here, or resolve a non-ambiguous magic Lore. "))
			]),
			_: 1
		}));
	}
	...
	props: { bridge: {} },
	setup(e) {
		let { ambiguousGrants: t, confirmMagicLorePrompt: n, dismissMagicLorePrompt: r, errorMessage: i, handleSpellDrop: a, initialize: o, isLoadingLoreOptions: s, isLoadingSpells: c, loadDetectedSpells: l, magicGrants: u, openMagicLorePrompt: d, pendingMagicLorePrompt: f, removeCustomSpell: p, resolvedGrantCount: m, selectedSpells: h, setSpellSelected: g, spells: _ } = TL(e.bridge);
		kr(() => {
			o();
		});
		function v(e, t) {
...
}), nR = { class: "dui-collapse-title" }, rR = { class: "dui-badge" }, iR = {
	key: 0,
	class: "dui-badge dui-badge-info"
}, aR = {
	key: 1,
	class: "dui-badge dui-badge-warning"
}, oR = { class: "dui-collapse-content" }, sR = { class: "dui-fieldset" }, cR = { class: "dui-fieldset-legend" }, lR = [
	"aria-label",
	"value",
	"onInput"
], uR = {
	...
	setup(e) {
		...
		return (t, o) => (L(), z(qj, {
			...
			default: N(() => [(L(!0), R(I, null, F(M(n), (t) => (L(), R("details", {
				...
				B("strong", null, k(t.name), 1),
				B("span", rR, k(r(t)), 1),
				t.config.rollable ? (L(), R("span", iR, "Rollable")) : U("", !0),
				t.config.damage ? (L(), R("span", aR, "Damage")) : U("", !0)
			]), B("div", oR, [
				B("fieldset", sR, [B("legend", cR, k(t.config.damage ? "Damage" : "Specification"), 1), B("input", {
				...
				t.config.rollable && !t.config.damage ? (L(), R("fieldset", uR, [o[0] ||= B("legend", { class: "dui-fieldset-legend" }, "Difficulty", -1), B("select", {
					...
					key: e.value,
					value: e.value
				}, k(e.label), 9, fR))), 128))], 40, dR)])) : U("", !0),
				t.config.damage && t.config.dice ? (L(), R("fieldset", pR, [o[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Dice", -1), B("input", {
			...
			])]))), 128)), M(n).length ? U("", !0) : (L(), R("p", gR, "No traits are selected yet."))]),
			_: 1
		}));
	}
}), vR = "__blank-item__";
...
function CR(e) {
	return e.resolution.status === "fallback" ? vR : e.resolution.selectedCandidateUuid;
}
function wR(e) {
	return e.source === "career";
}
function TR(e) {
	return e.resolution.candidates.length > 0 || wR(e);
}
function ER(e) {
...
var DR = {
	...
	"checked",
	"onChange"
], kR = { class: "dui-list-col-grow app:grid app:gap-2" }, AR = { key: 0 }, jR = {
	key: 1,
	class: "dui-fieldset"
}, MR = [
	"aria-label",
	...
	key: 0,
	value: ""
}, PR = ["value"], FR = ["value"], IR = { key: 2 }, LR = { class: "dui-card-actions" }, RR = { class: "dui-fieldset" }, zR = [
	"aria-label",
	"value",
	"onInput"
], BR = ["onClick"], VR = {
	...
	setup(e, { emit: t }) {
		let n = t;
		return (t, r) => e.trappings.length ? (L(), R("ul", DR, [(L(!0), R(I, null, F(e.trappings, (e) => (L(), R("li", {
			key: e.key,
			class: "dui-list-row"
		}, [B("input", {
			"aria-label": `Use ${e.name}`,
			...
			B("strong", null, k(e.name), 1),
			B("span", null, k(e.resolution.selectedItemType || e.itemType || "trapping") + " · " + k(M(yR)(e)), 1),
			M(ER)(e) ? (L(), R("span", AR, k(M(ER)(e)), 1)) : U("", !0),
			B("span", { class: O(M(SR)(e)) }, k(M(xR)(e)), 3),
			M(TR)(e) ? (L(), R("fieldset", jR, [r[0] ||= B("legend", { class: "dui-fieldset-legend" }, "Resolution", -1), B("select", {
				"aria-label": `Resolution for ${e.name}`,
				class: "dui-select dui-select-sm",
				value: M(CR)(e),
				onChange: (t) => n("resolutionChange", e.key, t)
			}, [
				e.resolution.candidates.length ? (L(), R("option", NR, "Choose match")) : U("", !0),
				(L(!0), R(I, null, F(e.resolution.candidates, (e) => (L(), R("option", {
					key: e.uuid,
					value: e.uuid
				}, k(e.name) + " (" + k(e.sourceLabel) + ") ", 9, PR))), 128)),
				M(wR)(e) ? (L(), R("option", {
					key: 1,
					value: M(vR)
				}, " Blank Item ", 8, FR)) : U("", !0)
			], 40, MR)])) : (L(), R("span", IR, k(M(bR)(e)), 1)),
			B("div", LR, [B("fieldset", RR, [r[1] ||= B("legend", { class: "dui-fieldset-legend" }, "Quantity", -1), B("input", {
			...
			}, null, 40, zR)]), e.source === "custom" ? (L(), R("button", {
				key: 0,
				class: "dui-btn dui-btn-sm",
				type: "button",
				onClick: (t) => n("removeCustomTrapping", e.key)
			}, " Remove ", 8, BR)) : U("", !0)])
		])]))), 128))])) : (L(), R("p", VR, "No trappings are selected yet."));
	}
}), UR = { class: "dui-card-actions" }, WR = ["disabled"], GR = { key: 0 }, KR = {
	key: 0,
	class: "dui-alert dui-alert-error",
	role: "alert"
}, qR = /* @__PURE__ */ P({
	...
	props: { bridge: {} },
	setup(e) {
		let t = e, n = uj(), { trappings: r } = oc(n), i = /* @__PURE__ */ j(""), a = /* @__PURE__ */ j(!1), o = W(() => r.value.filter((e) => !e.ignored && e.resolution.status === "unresolved"));
		kr(() => {
			u();
		});
		function s(e, t) {
		...
		return (e, t) => (L(), z(qj, {
			...
			default: N(() => [
				B("div", UR, [B("button", {
					...
					type: "button",
					onClick: u
				}, k(a.value ? "Resolving..." : "Resolve Trappings"), 9, WR), o.value.length ? (L(), R("span", GR, k(o.value.length) + " unresolved ", 1)) : U("", !0)]),
				i.value ? (L(), R("p", KR, k(i.value), 1)) : U("", !0),
				V(HR, {
					trappings: M(r),
					onQuantityInput: s,
...
function JR(e, t) {
	...
	for (let a of e) {
		let e = /* @__PURE__ */ new Map();
		for (let o of NO(a.career.uuid, a.career.grants.skills)) {
			let s = MO(o.originalName);
			if (!s) continue;
			let c = PO(o.originalName), l = n.get(c) ?? [], u = e.get(c) ?? 0, d = t.enableLinkedSkillResolution && l[u] ? l[u] : "";
			if (e.set(c, u + 1), d) {
				r.push({
					linkedFromKey: d,
			...
			i.push({
				alreadyGrantedSpecializations: $R(a.career.grants.skills, s.baseName),
				baseName: s.baseName,
				careerLabel: ez(a.career),
				isLoadingSuggestions: !1,
				occurrence: o.occurrence,
				options: s.options,
				originalName: s.originalName,
				resolvedSpecialization: tz(s),
				resolutionKey: o.resolutionKey,
...
function YR(e) {
	return e.resolvedSpecialization.trim() ? AO(e.baseName, e.resolvedSpecialization) : "";
}
function XR(e) {
...
function QR(e, t) {
	let n = PO(t);
	return e.alreadyGrantedSpecializations.some((e) => PO(e) === n);
}
function $R(e, t) {
	let n = PO(t), r = /* @__PURE__ */ new Set(), i = [];
	for (let t of e) {
		let e = jO(t);
		if (!e || PO(e.baseName) !== n) continue;
		let a = PO(e.specialization);
...
var nz = { class: "dui-card-body" }, rz = { class: "dui-card-title" }, iz = { class: "dui-badge" }, az = { class: "dui-fieldset" }, oz = { class: "app:grid app:gap-1" }, sz = ["onUpdate:modelValue", "aria-label"], cz = ["value"], lz = [
	"onUpdate:modelValue",
	"aria-label",
	"placeholder"
], uz = {
	...
	key: 0,
	class: "dui-card-actions"
}, fz = { key: 0 }, pz = ["onClick"], mz = {
	key: 0,
	class: "dui-badge dui-badge-error dui-badge-xs"
}, hz = {
	key: 0,
	class: "dui-alert dui-alert-info"
}, gz = { class: "dui-card-actions" }, _z = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		let n = t;
		function r(e) {
			return !!e.resolvedSpecialization && QR(e, e.resolvedSpecialization);
		}
		return (t, i) => (L(), R("section", null, [
			...
			(L(!0), R(I, null, F(e.prompt.rows, (t) => (L(), R("section", {
				...
				B("h3", rz, k(e.getSkillResolutionLabel(t)), 1),
				B("span", iz, k(t.careerLabel), 1),
				B("fieldset", az, [
					i[4] ||= B("legend", { class: "dui-fieldset-legend" }, "Specialization", -1),
					B("label", oz, [i[3] ||= B("span", { class: "dui-label" }, "Choice", -1), t.options.length > 1 ? Vn((L(), R("select", {
						key: 0,
						"onUpdate:modelValue": (e) => t.resolvedSpecialization = e,
						"aria-label": `Specialization for ${e.getSkillResolutionLabel(t)}`,
						class: O(["dui-select dui-select-sm", { "dui-select-error": M(QR)(t, t.resolvedSpecialization) }])
					}, [i[2] ||= B("option", { value: "" }, "Leave unresolved", -1), (L(!0), R(I, null, F(t.options, (e) => (L(), R("option", {
						key: e,
						class: O({ "app:text-error": M(QR)(t, e) }),
						value: e
					}, k(e) + k(M(QR)(t, e) ? " — already granted" : ""), 11, cz))), 128))], 10, sz)), [[Wo, t.resolvedSpecialization]]) : Vn((L(), R("input", {
						key: 1,
						"onUpdate:modelValue": (e) => t.resolvedSpecialization = e,
						"aria-label": `Specialization for ${e.getSkillResolutionLabel(t)}`,
						class: O(["dui-input dui-input-sm", { "dui-input-error": M(QR)(t, t.resolvedSpecialization) }]),
						placeholder: t.suggestedSpecializations.length ? "Type or choose below" : t.specialization,
						type: "text"
					}, null, 10, lz)), [[Uo, t.resolvedSpecialization]])]),
					r(t) ? (L(), R("p", uz, " Already granted by this Career. ")) : U("", !0)
				]),
				e.usesFreeformSkillSpecialization(t) ? (L(), R("div", dz, [t.isLoadingSuggestions ? (L(), R("small", fz, "Finding known choices.")) : U("", !0), (L(!0), R(I, null, F(t.suggestedSpecializations, (e) => (L(), R("button", {
					key: `${t.resolutionKey}:${e}`,
					class: O(["dui-btn dui-btn-sm", { "dui-btn-error dui-btn-outline": M(QR)(t, e) }]),
					type: "button",
					onClick: (r) => n("chooseSkillSpecialization", t, e)
				}, [H(k(e) + " ", 1), M(QR)(t, e) ? (L(), R("span", mz, " Already granted ")) : U("", !0)], 10, pz))), 128))])) : U("", !0)
			])]))), 128)),
			e.prompt.linkedRows.length ? (L(), R("div", hz, k(e.prompt.linkedRows.length) + " linked skill specialization" + k(e.prompt.linkedRows.length === 1 ? "" : "s") + " will reuse earlier choices from this career chain. ", 1)) : U("", !0),
			B("div", gz, [B("button", {
				class: "dui-btn dui-btn-sm",
				type: "button",
...
var Sz = ["aria-current", "onClick"], Cz = ["aria-current", "popovertarget"], wz = ["id"], Tz = ["onClick"], Ez = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, r) => (L(), R(I, null, [r[0] ||= B("span", { class: "dui-megamenu-active" }, null, -1), (L(!0), R(I, null, F(e.groups, (t) => (L(), R(I, { key: t.key }, ["page" in t ? (L(), R("button", {
		...
		}, k(t.label), 9, Sz)) : (L(), R(I, { key: 1 }, [B("button", {
			"aria-current": t.isActive ? "page" : void 0,
			popovertarget: t.popoverId,
			type: "button"
		}, k(t.label), 9, Cz), B("div", {
			id: t.popoverId,
			popover: ""
			...
			type: "button",
			onClick: (e) => n("pageSelect", t.page, e)
		}, k(M(xz)(t.page)), 11, Tz)]))), 128))], 2)], 8, wz)], 64))], 64))), 128))], 64));
	}
}), Dz = { class: "dui-navbar app:sticky app:top-0 app:z-20 app:flex-wrap app:gap-2 app:bg-base-200 app:px-3 app:py-2" }, Oz = { class: "dui-navbar-start app:min-w-64 app:flex-1" }, kz = { class: "app:min-w-0" }, Az = { class: "app:text-base-content/70" }, jz = {
	...
	popover: ""
}, Nz = { class: "dui-navbar-end app:w-auto app:shrink-0" }, Pz = ["disabled"], Fz = /* @__PURE__ */ P({
	...
	setup(e, { emit: t }) {
		...
		return (t, n) => (L(), R("header", Dz, [
			B("div", Oz, [B("div", kz, [
				...
				B("small", Az, [e.selectedBaseActorName ? (L(), R(I, { key: 0 }, [H(k(e.selectedBaseActorName) + " base · " + k(e.finalActorName), 1)], 64)) : (L(), R(I, { key: 1 }, [H("Choose a base character, then shape the final NPC.")], 64))])
			])]),
			B("nav", jz, [n[3] ||= B("button", {
				"aria-label": "Open NPC Builder navigation",
				class: "dui-btn dui-btn-sm sm:app:hidden",
				popovertarget: "npc-builder-megamenu",
				type: "button"
			}, " Menu ", -1), B("div", Mz, [V(Ez, {
...
function Iz(e, t, n, r) {
	let i = uj(), a = /* @__PURE__ */ j(!1);
	function o(e) {
		Lz(e) || (e.preventDefault(), a.value = !0);
	}
	function s(e) {
		if (Lz(e)) return;
		let t = e.currentTarget, n = e.relatedTarget;
	...
	function c(e) {
		Lz(e) || (e.preventDefault(), e.dataTransfer && (e.dataTransfer.dropEffect = "copy"));
	}
	async function l(o) {
		if (!Lz(o)) {
...
function Lz(e) {
	...
	return n.startsWith("npc-builder-career:") || Zk(n) !== null || r.includes("application/x-wfrp4e-customizer-portrait-filter-tag");
}
//#endregion
//#region src/view/apps/npc-builder/NpcBuilderApp/useNpcBuilderBuild.ts
function Rz(e, t, n, r, i) {
	let a = uj(), { advancements: o, buildTraits: s, careers: c, finalActorName: l, finalPortraitPath: u, selectedMountActorUuid: d, selectedBaseActor: f, selectedSpells: p, settings: m, trappings: h } = oc(a), g = /* @__PURE__ */ j(!1), _ = W(() => !!(f.value && c.value.length && !g.value && !i.value));
	async function v() {
...
function Kz(e) {
	let t = uj(), { careers: n, settings: r } = oc(t), i = /* @__PURE__ */ j(""), a = /* @__PURE__ */ j(""), o = /* @__PURE__ */ j(!1), s = /* @__PURE__ */ j(null), c = /* @__PURE__ */ j(null), l = W(() => zz(s.value));
	...
	function v() {
		let e = c.value;
		if (e) {
			for (let n of e.rows) t.setSkillGrantResolution(n.resolutionKey, YR(n));
	...
	return {
		...
		usesFreeformSkillSpecialization: ZR
	};
}
function qz(e) {
	return e instanceof Error ? e.message : "The NPC Builder could not finish that action.";
...
function Yz(e, t) {
	let n = uj(), { selectedBaseActorUuid: r, selectedMountActorUuid: i, settings: a } = oc(n), o = /* @__PURE__ */ j(!1), s = /* @__PURE__ */ j(!1), c = /* @__PURE__ */ j([]);
	kr(async () => {
		o.value = !0;
		try {
		...
		try {
			...
			t.value = WF(e), n.clearBaseDraftData(), n.hydrateBaseActorCombatProfile(null);
		} finally {
			s.value = !1;
		}
	});
...
function Zz(e) {
	let t = /* @__PURE__ */ new Set();
	for (let n of e) n.kind === "skill" && !n.characteristicKey && !MO(n.name) && t.add(n.name);
	return [...t];
}
function Qz(e) {
	let t = /* @__PURE__ */ new Set();
...
function eB(e, t) {
	return {
		...e,
		inFlightNames: rB([...e.inFlightNames, ...t])
	};
}
function tB(e, t) {
	let n = new Set(rB(t));
	return {
		...
		successfulNames: rB([...e.successfulNames, ...n])
	};
}
function nB(e, t) {
	let n = new Set(rB(t));
...
function rB(e) {
	return [...new Set([...e].map(PO).filter(Boolean))];
}
//#endregion
//#region src/state/npc-builder/workflows/metadata-lookups-workflow.ts
function iB(e) {
	let t = uj(), { advancements: n } = oc(t), r = /* @__PURE__ */ j(Xz()), i = /* @__PURE__ */ j(Xz()), a = /* @__PURE__ */ j(""), o = /* @__PURE__ */ j(""), s = W(() => Zz(n.value)), c = W(() => Qz(n.value)), l = W(() => [a.value, o.value].filter(Boolean).join(" ")), u = W(() => l.value ? "degraded" : r.value.inFlightNames.length + i.value.inFlightNames.length > 0 ? "loading" : "ready");
	Jn(s, (e) => {
		d(e);
	}, { immediate: !0 }), Jn(c, (e) => {
		f(e);
	...
	async function d(n) {
		...
		let i = $z(n, r.value);
		if (i.length) {
			r.value = eB(r.value, i), a.value = "";
			try {
				let n = await e.listSkillCharacteristics(i);
				r.value = tB(r.value, i), t.hydrateSkillCharacteristics(n);
			} catch (e) {
				r.value = nB(r.value, i), a.value = aB("skill characteristics", e);
			}
		}
	}
	async function f(n) {
		...
		let r = $z(n, i.value);
		if (r.length) {
			i.value = eB(i.value, r), o.value = "";
			try {
				let n = await e.listTalentMaximums(r);
				i.value = tB(i.value, r), t.hydrateTalentMaximums(n);
			} catch (e) {
				i.value = nB(i.value, r), o.value = aB("Talent maximums", e);
			}
		}
	}
	async function p() {
...
var sB = ["id", "aria-label"], cB = {
	...
	props: { bridge: {} },
	setup(e) {
		let t = e, { finalActorName: n, hasMagicAccess: r, selectedBaseActor: i, selectedSpells: a } = oc(uj()), o = /* @__PURE__ */ j("build-quick"), s = mr(), c = W(() => r.value || a.value.length > 0), { addCareerSummaryWithLowerCareerMode: l, buildMessage: u, chooseSkillSpecialization: d, confirmLowerCareerPrompt: f, confirmSkillResolutionPrompt: p, dismissLowerCareerPrompt: m, dismissSkillResolutionPrompt: h, errorMessage: g, getSkillResolutionLabel: _, isCareerQueued: v, isFindingLowerCareers: y, isLowerCareerSelected: b, lowerCareerCandidateGroups: x, pendingLowerCareerPrompt: S, pendingSkillResolutionPrompt: C, setLowerCareerSelected: ee, usesFreeformSkillSpecialization: te } = Jz(t.bridge), { buildNpc: ne, canBuild: re } = Rz(t.bridge, o, u, g, y), { isLoadingActors: ie, isLoadingBaseDraft: w, traitDifficultyOptions: T } = Yz(t.bridge, g), { metadataLookupError: ae, metadataLookupStatus: oe, retryMetadataLookups: E } = oB(t.bridge), { handleApplicationDragEnter: se, handleApplicationDragLeave: D, handleApplicationDragOver: ce, handleApplicationDrop: le, isApplicationDragOver: ue } = Iz(t.bridge, o, l, g);
		return (e, r) => (L(), R("section", {
			...
			B("section", {
				id: `${M(s)}-panel`,
				"aria-label": M(xz)(o.value),
				class: "app:grid app:flex-1 app:content-start app:gap-3 app:p-3"
			}, [
				M(g) ? (L(), R("p", cB, k(M(g)), 1)) : M(u) ? (L(), R("p", lB, k(M(u)), 1)) : M(ue) ? (L(), R("p", uB, " Release to add this document to the NPC draft. ")) : U("", !0),
				M(oe) === "degraded" ? (L(), R("div", dB, [
				...
				M(yz)(o.value) ? (L(), z(vL, {
					...
					bridge: t.bridge,
					page: o.value
				}, null, 8, ["bridge", "page"])) : M(bz)(o.value) ? (L(), z(DM, {
					key: 5,
					page: o.value
				}, null, 8, ["page"])) : o.value === "trappings" ? (L(), z(qR, {
					key: 6,
					bridge: t.bridge
				}, null, 8, ["bridge"])) : o.value === "traits" ? (L(), z(_R, {
					key: 7,
					"difficulty-options": M(T)
				}, null, 8, ["difficulty-options"])) : o.value === "detail-spells" ? (L(), z(tR, {
					key: 8,
					bridge: t.bridge
				}, null, 8, ["bridge"])) : o.value === "mount" ? (L(), z(hI, {
					key: 9,
					bridge: t.bridge
				}, null, 8, ["bridge"])) : M(vz)(o.value) ? (L(), z(xF, {
					key: 10,
					bridge: t.bridge,
...
function pB(e) {
	return {
		characteristics: mB(e),
		skills: hB(e),
		talents: _B(e, [["talents", "value"], ["talents"]]),
		trappings: _B(e, [["trappings", "value"], ["trappings"]])
	};
}
function mB(e) {
	let t = _B(e, [["characteristics", "value"], ["characteristics"]]);
	if (t.length) return t.map(gB);
	let n = Y(e, ["characteristics"]);
	if (!J(n)) return [];
	let r = [];
	for (let [e, t] of Object.entries(n)) t && r.push(gB(e));
	return yB(r);
}
function hB(e) {
	return _B(e, [["skills", "value"], ["skills"]], { preserveDuplicates: !0 });
}
function gB(e) {
	let t = e.trim().toLocaleLowerCase();
	if (Ac(t)) return Oc[t];
...
function _B(e, t, n = {}) {
	for (let r of t) {
		let t = zl(Y(e, r));
		if (t.length) return n.preserveDuplicates ? vB(t) : yB(t);
	}
	return [];
}
function vB(e) {
...
var CB = [
	...
	"system.talents",
	"system.trappings"
], wB = /* @__PURE__ */ new Map(), TB = "idle", EB = null;
function DB() {
	return EB || (TB = "indexing", wB.clear(), EB = kB().then(() => {
		TB = "ready";
	}).catch((e) => {
		TB = "error", t("wfrp4e-customizer-apps | Career indexing failed.", e);
	}), EB);
}
async function OB(e) {
	return TB === "idle" && DB(), !e.careerGroup || e.level === null ? [] : [...wB.values()].filter((t) => PB(t, e)).sort(IB);
}
async function kB() {
	AB(), await QE();
	for (let e of game.packs ?? []) {
		if (!YE(e) || !e.getIndex) continue;
		let t = await e.getIndex({ fields: CB });
		for (let n of ZE(t)) {
			let t = jB(e, n);
			t && wB.set(t.uuid, t);
		}
...
function AB() {
	for (let e of game.items?.contents ?? []) e.type === "career" && wB.set(e.uuid, bB(e));
}
function jB(e, t) {
	let n = JE(e, t);
...
function PB(e, t) {
	return e.uuid !== t.uuid && e.level !== null && t.level !== null && e.level < t.level && FB(e.careerGroup) === FB(t.careerGroup);
}
function FB(e) {
	return e.trim().toLocaleLowerCase();
}
...
], RB = /* @__PURE__ */ new Map(), zB = /* @__PURE__ */ new Map(), BB = /* @__PURE__ */ new Map(), VB = "idle", HB = null;
async function UB(e) {
	let t = PO(e);
	return t ? (VB === "idle" && GB(), HB && await HB, [...RB.get(t) ?? []].sort((e, t) => e.localeCompare(t))) : [];
}
async function WB(e) {
	return VB === "idle" && GB(), HB && await HB, e.flatMap((e) => {
		let t = ZB(e);
		return t ? [{
			...t,
			skillName: e
...
function GB() {
	return HB || (VB = "indexing", RB.clear(), zB.clear(), BB.clear(), HB = KB().then(() => {
		VB = "ready";
	}).catch((e) => {
		VB = "error", t("wfrp4e-customizer-apps | Skill specialization indexing failed.", e);
	}), HB);
}
async function KB() {
	QB(), await QE();
	for (let e of game.packs ?? []) {
		if (!YE(e) || !e.getIndex) continue;
		let t = await e.getIndex({ fields: LB });
		for (let e of ZE(t)) JB(e);
		await QE();
	}
}
function qB(e) {
	...
	let t = jO(e.name);
	if (!t) return;
	let n = PO(t.baseName), r = RB.get(n) ?? /* @__PURE__ */ new Set();
	r.add(t.specialization), RB.set(n, r);
}
function JB(e) {
...
function YB(e) {
	...
	let n = {
		...
		skillName: e.name
	}, r = PO(e.name), i = PO(jO(e.name)?.baseName ?? e.name);
	zB.set(r, n), BB.has(i) || BB.set(i, n);
}
function XB(e) {
	let t = X(e, [
		"system",
...
function ZB(e) {
	let t = PO(e), n = PO(jO(e)?.baseName ?? e);
	return zB.get(t) ?? BB.get(n) ?? null;
}
function QB() {
	for (let e of game.items?.contents ?? []) qB(e);
}
//#endregion
...
function eV(e, t, n) {
	let r = e ? e.toObject() : $B(t, n);
	return delete r._id, r;
}
function tV(e, t, n) {
	return nV(e, t, n)[0] ?? null;
}
function nV(e, t, n) {
	return e.items?.contents.filter((e) => e.type === n && aV(e.name, t)) ?? [];
}
function rV(e, t, n) {
	return e.items?.contents.find((e) => t && e.uuid === t ? !0 : aV(e.name, n)) ?? null;
}
function iV(e, t) {
	return game.items?.contents.find((n) => t.includes(n.type) && aV(n.name, e)) ?? null;
}
function aV(e, t) {
...
async function oV(e, t) {
	return await game.wfrp4e?.utility?.findItem?.(e, t) || iV(e, t);
}
//#endregion
...
function uV(e) {
	let t = lA();
	return fV(e) ? {
		allowBaseActorCharacteristics: pV(e.allowBaseActorCharacteristics, lV.allowBaseActorCharacteristics),
		allowBaseActorSkills: pV(e.allowBaseActorSkills, lV.allowBaseActorSkills),
		allowBaseActorTalents: pV(e.allowBaseActorTalents, lV.allowBaseActorTalents),
		allowBaseActorTraits: pV(e.allowBaseActorTraits, lV.allowBaseActorTraits),
		allowBaseActorTrappings: pV(e.allowBaseActorTrappings, lV.allowBaseActorTrappings),
		askForLinkedSkillSpecializations: pV(e.askForLinkedSkillSpecializations, lV.askForLinkedSkillSpecializations),
		autoSelectGrantedSpells: pV(e.autoSelectGrantedSpells, lV.autoSelectGrantedSpells),
		baseActorFolderUuid: mV(e.baseActorFolderUuid, lV.baseActorFolderUuid),
		includeSpeciesInName: pV(e.includeSpeciesInName, lV.includeSpeciesInName),
		lowerCareerMode: dV(e.lowerCareerMode) ? e.lowerCareerMode : lV.lowerCareerMode,
		outputActorFolderUuid: mV(e.outputActorFolderUuid, lV.outputActorFolderUuid),
		quickTraitFolderUuid: mV(e.quickTraitFolderUuid, lV.quickTraitFolderUuid),
		searchCompendiumPortraitAssets: pV(e.searchCompendiumPortraitAssets, lV.searchCompendiumPortraitAssets),
		searchFoundryPortraitAssets: pV(e.searchFoundryPortraitAssets, lV.searchFoundryPortraitAssets),
		searchWebPortraitAssets: pV(e.searchWebPortraitAssets, lV.searchWebPortraitAssets)
	} : t;
}
function dV(e) {
...
async function SV(e, t) {
	...
	for (let i of t) {
		...
		let s = eV(await jV(i), i.name, i.kind);
		s.type = i.kind, Bl(s, [
			"system",
			"advances",
			"value"
	...
	Object.keys(n).length && await e.update(n), await bV(e, r);
}
function CV(e) {
...
function TV(e, t) {
	return t === "talent" ? EV(e) : e.items?.contents.filter((e) => e.type === t).map((n) => DV(e, n, t)) ?? [];
}
function EV(e) {
	let t = /* @__PURE__ */ new Map();
...
function DV(e, t, n) {
	...
	let a = Il(r, [["modifier", "value"], ["modifier"]]), o = OV(r), s = {
		baseAdvances: i,
		baseModifier: a,
		current: (o ? xV(e.toObject().system, o) : 0) + i + a,
		kind: n,
		name: t.name
	};
...
async function AV(e, t, n, r) {
	let i = Math.max(0, t.baseAdvances + n), a = nV(e, t.name, "talent"), o = a[0] ?? await jV(t);
	r.deletes.push(...a.map((e) => e.id));
	for (let e = 0; e < i; e += 1) {
		let e = eV(o, t.name, "talent");
		e.type = "talent", Bl(e, [
			"system",
			"advances",
			"value"
...
function PV(e) {
	return {
		...vO(),
		attackType: RV(e.system, ["rollable", "attackType"]) || "melee",
		bonusCharacteristic: RV(e.system, ["rollable", "bonusCharacteristic"]),
		damage: Rl(e.system, [["rollable", "damage"]]),
		defaultDifficulty: RV(e.system, ["rollable", "defaultDifficulty"]) || "challenging",
		dice: RV(e.system, ["rollable", "dice"]),
		rollable: Rl(e.system, [["rollable", "value"]]),
		skill: RV(e.system, ["rollable", "skill"]),
		sl: Rl(e.system, [["rollable", "SL"]], !0),
		specification: RV(e.system, ["specification", "value"])
	};
}
function FV(e) {
	return LV(e.system);
...
async function zV(e, t) {
	let n = yV();
	for (let r of t) {
		let t = r.source === "base" ? rV(e, r.sourceUuid, r.name) : tV(e, r.name, "trait");
		if (r.ignored) {
			t && n.deletes.push(t.id);
			continue;
		}
		if (t) {
			n.updates.push(NV(t.id, r.config));
			continue;
		}
		let i = eV(r.sourceUuid ? await BV(r.sourceUuid) : await oV(r.name, ["trait"]), r.name, "trait");
		i.type = "trait", Bl(i, ["system", "disabled"], !1), MV(i, r.config), n.creates.push(i);
	}
	await bV(e, n);
}
async function BV(e) {
...
function VV(e) {
	return e.items?.contents.filter((e) => e.type === "trait" && !FV(e)).map(WV) ?? [];
}
function HV(e) {
	return e.items?.contents.filter((e) => e.type === "trait" && FV(e)).map(WV) ?? [];
}
...
async function qV(e) {
	...
	let n = oE(await fromUuid(t.uuid), "trait", "Drop a Foundry Trait item here.");
	return {
		config: PV(n),
		ignored: !1,
		key: `custom:${n.uuid}`,
...
async function YV(e) {
	return nH(await tH(e, "Actor"));
}
async function XV(e) {
	return nH(await tH(e, "Item"));
}
function ZV() {
	return game.folders.contents.filter((e) => e.type === "Actor").map(nH).sort((e, t) => e.name.localeCompare(t.name));
}
function QV() {
	return game.folders.contents.filter((e) => e.type === "Item").map(nH).sort((e, t) => e.name.localeCompare(t.name));
}
function $V(e) {
...
async function tH(e, t) {
	...
	let r = game.folders.contents.find((e) => e.type === t && rH(e.name, n));
	if (r) return r;
...
async function iH(e) {
	let t = eH(e.quickTraitFolderUuid);
	if (!t) throw Error("Choose a Quick Traits item folder before importing traits.");
	let n = new Set(sH(e).map((e) => e.name.trim().toLocaleLowerCase()));
	for (let e of JV) {
		if (n.has(e.trim().toLocaleLowerCase())) continue;
		let r = eV(await oV(e, ["trait"]), e, "trait");
		r.folder = t.id, r.type = "trait", await Item.create(r);
	}
	return ui.notifications?.info("Imported recommended quick traits."), await aH(e);
}
async function aH(e) {
	return sH(e).map(cH).sort((e, t) => e.name.localeCompare(t.name));
}
function oH(e, t) {
	return t.quickTraitFolderUuid ? e.folder?.uuid === t.quickTraitFolderUuid : !1;
}
function sH(e) {
	return game.items?.contents.filter((t) => t.type === "trait" && oH(t, e)) ?? [];
}
function cH(e) {
	return {
		config: PV(e),
...
async function uH(e, t) {
	let n = yV();
	for (let r of t) {
		let t = r.source === "base" ? rV(e, r.sourceUuid, r.name) : null;
		if (r.ignored) {
			t && n.deletes.push(t.id);
			continue;
		}
		...
		let i = await gH(r), a = r.resolution.selectedItemType || r.itemType || "trapping", o = eV(i, r.resolution.selectedName || r.name, a);
...
async function dH(e) {
	return KA(e, await _H());
}
async function fH(e) {
	let t = sE(e);
	if (t.type !== "Item" || !t.uuid) throw Error("Drop a Foundry Item here.");
	let n = aE(await fromUuid(t.uuid), "Drop a Foundry Item here.");
	return {
		ignored: !1,
		itemType: n.type,
		key: `custom:${n.uuid}`,
...
async function gH(e) {
	...
	return e.resolution.status === "fallback" ? null : await oV(e.resolution.selectedName || e.name, hH());
}
async function _H() {
	let e = [], t = hH();
	for (let n of game.items?.contents ?? []) t.includes(n.type) && e.push(yH(n, "World"));
	for (let n of game.packs ?? []) {
		...
		for (let r of i) rE(r) && t.includes(r.type) && e.push(yH(r, n.title ?? "Compendium"));
	}
	return e;
}
async function vH(e, t) {
	return e.getIndex ? ZE(await e.getIndex({ fields: ["name", "type"] })).filter((n) => !!(n.name && n.type && JE(e, n) && t.includes(n.type))).map((t) => ({
...
function bH(e) {
	return game.actors.contents.filter((t) => EH(t, e)).map(CH);
}
async function xH(e) {
	let t = iE(await fromUuid(e));
	return {
		advancements: [
			...wV(t),
			...TV(t, "skill"),
...
async function SH(e) {
	...
	let n = null;
	return t.uuid ? n = await fromUuid(t.uuid) : t.id && (n = game.actors.get(t.id)), CH(iE(n));
}
function CH(e) {
	return {
		img: e.img ?? "",
		name: e.name,
		prototypeTokenImg: TH(e),
		species: wH(e),
		type: e.type,
		uuid: e.uuid
	};
...
async function DH(e) {
	...
	return bB(oE(await fromUuid(t.uuid), "career", "Drop a WFRP Career item here."));
}
async function OH(e) {
	let t = [];
	for (let n of e) {
		let e = oE(await fromUuid(n.uuid), "career", `Career “${n.name}” is no longer available.`);
		for (let r = 0; r < EO(n.quantity); r += 1) {
			let n = e.toObject();
			delete n._id, Bl(n, [
				"system",
				"complete",
...
var AH = "spell", jH = new Set(pA), MH = new Set(mA);
async function NH() {
	return PH().map((e) => ({
		category: yA(e.key),
		key: e.key,
...
function PH() {
	...
	return n.some((e) => e.key === "petty") || n.push(RH()), n;
}
function FH(e, t) {
	let n = /* @__PURE__ */ new Map();
	for (let r of e) {
		...
		if (r.kind === "petty-magic") {
			let e = HH("petty magic", t);
			e && n.set(e.key, e);
			continue;
		}
		let e = HH(r.rawLore, t);
		e && n.set(e.key, e);
	}
	return [...n.values()];
}
function IH(e, t) {
	let n = [...LH(e.system), VH(e.name)].filter(Boolean);
	for (let e of n) {
		let n = BH(e, t);
		if (n) return n;
		let r = HH(e, t);
...
function BH(e, t) {
	let n = gA(e);
	return n === "lore" ? t.find((e) => e.key !== "petty") ?? null : n === "the eight winds" || n === "eight winds" ? t.find((e) => jH.has(e.key)) ?? null : n === "dark lore" ? t.find((e) => MH.has(e.key)) ?? null : null;
}
function VH(e) {
...
function HH(e, t) {
	let n = gA(e);
	return n ? t.find((e) => e.matchTerms.some((e) => gA(e) === n)) ?? null : null;
}
function UH(e, t, n) {
	let r = /* @__PURE__ */ new Set(), i = gA(e), a = gA(t);
	for (let i of [
		e,
		t,
		n
...
var KH = "[Drowsy's WFRP4e Customizers][Spell Lookup]";
function qH(t, n) {
	if (n) {
		e(`${KH} ${t}`, n);
		return;
...
function $H(e) {
	return /^item\./i.test(e.uuid) ? "World" : eU(e.uuid, "WFRP Item Lookup");
}
function eU(e, t) {
...
async function tU(e) {
	if (qH("Compendium index scan start", { pack: YH(e) }), !e.getIndex) return qH("Compendium has no index; loading documents", { pack: YH(e) }), await iU(e);
	let t = ZE(await e.getIndex({ fields: [
		"name",
		"type",
		"img",
		"system.lore.value"
	] }));
	if (qH("Compendium index loaded", {
		entries: t.length,
		pack: YH(e),
		samples: t.slice(0, 5).map((t) => ({
			hasLoreTerms: LH(t).length > 0,
			name: t.name,
			type: t.type,
			uuid: JE(e, t)
		}))
	}), !t.length) return qH("Compendium index empty; loading documents", { pack: YH(e) }), await iU(e);
	let n = t.filter(rU);
	qH("Compendium index spell candidates", {
		pack: YH(e),
		spellEntries: n.length
	});
	let r = n.filter((e) => e.name).map((t) => oU(e, t));
	return r.length || !aU(e) ? r : await iU(e);
}
function nU(e) {
	return YE(e);
}
function rU(e) {
	return e.type === "spell" ? !0 : !!(e.name && (LH(e).length || VH(e.name)));
}
async function iU(e) {
	if (!e.getDocuments) return qH("Compendium has no document loader", { pack: YH(e) }), [];
	qH("Compendium document load start", { pack: YH(e) });
	let t = await e.getDocuments(), n = t.filter((e) => rE(e) && e.type === "spell");
	return qH("Compendium document load complete", {
		documents: t.length,
		pack: YH(e),
	...
	}), n.map((t) => QH(t, e.title ?? "Compendium"));
}
function aU(e) {
...
async function sU() {
	let e = lU();
	if (!e) return qH("WFRP helper unavailable"), [];
	try {
		let t = await e.findAllItems(AH, "Loading Spells", !0, ["system.lore.value"]);
		return qH("WFRP helper raw result", {
			count: t.length,
			samples: t.slice(0, 10).map(ZH)
		}), (await Promise.all(t.map((e) => cU(e)))).filter((e) => e !== null);
	} catch (e) {
		return JH("WFRP helper lookup failed.", e), [];
	}
}
async function cU(e) {
	if (typeof e == "string") {
		let t = await fromUuid(e);
		return rE(t) && t.type === "spell" ? QH(t, $H(t)) : null;
	}
	if (rE(e)) return e.type === "spell" ? QH(e, $H(e)) : null;
	if (X(e, ["type"]) !== "spell") return null;
	let t = X(e, ["name"]);
	return t ? {
		img: X(e, ["img"]) || X(e, ["thumb"]),
		name: t,
		sourceLabel: eU(X(e, ["uuid"]), "WFRP Item Lookup"),
		system: Y(e, ["system"]),
		uuid: X(e, ["uuid"])
	} : null;
}
...
async function uU() {
	let e = [], n = [...game.packs ?? []];
	qH("Candidate lookup start", {
		itemPacks: n.filter(nU).length,
		totalPacks: n.length,
		warhammerUtilityAvailable: !!pU(),
		worldItems: game.items?.contents.length ?? 0
	});
	let r = await sU();
	qH("WFRP helper lookup complete", {
		utilityInputs: r.length,
		utilitySamples: r.slice(0, 10).map(XH)
	}), e.push(...r), e.push(...dU()), qH("World spell scan complete", { worldSpellCount: e.filter((e) => e.sourceLabel === "World").length });
	for (let r of n) if (nU(r)) try {
		let t = await tU(r);
		e.push(...t), qH("Compendium spell scan complete", {
			inputCount: t.length,
			pack: YH(r),
			samples: t.slice(0, 5).map(XH)
		});
	} catch (e) {
		t(`wfrp4e-customizer-apps | Spell lookup skipped compendium "${r.title ?? r.collection ?? "unknown"}".`, e);
	}
	let i = fU(e);
	return qH("Candidate lookup complete", {
...
function dU() {
	let e = [];
	for (let t of game.items?.contents ?? []) t.type === "spell" && e.push(QH(t, "World"));
	return e;
}
function fU(e) {
	let t = /* @__PURE__ */ new Map();
...
async function mU(e, t) {
	let n = [];
	for (let r of t) {
		if (!r.selected || tV(e, r.name, "spell")) continue;
		let t = eV(r.sourceUuid ? await _U(r.sourceUuid) : null, r.name, AH);
		t.type = AH, n.push(t);
	}
...
async function hU(e) {
	...
	let n = await uU(), r = /* @__PURE__ */ new Map(), i = [];
	for (let e of n) {
		let n = IH(e, t);
		if (!n) {
			i.length < 20 && i.push({
	...
	return qH("Grant resolution complete", {
...
async function gU(e) {
	...
	let n = oE(await fromUuid(t.uuid), AH, "Drop a Foundry Spell item here."), r = IH(QH(n, "Dropped"), [...PH(), RH()]) ?? zH(LH(n.system)[0] ?? "");
	return {
		img: n.img ?? "",
		key: `custom:${n.uuid}`,
		loreKey: r.key,
...
function yU(e, t) {
	return t.traits.flatMap((t) => {
		...
		}), t.fixedDamage !== null && xU(r, t.fixedDamage), [r];
	});
}
function bU(e, t) {
...
async function SU(e, t, n, r) {
	let i = e.items?.contents.filter(TU) ?? [], a = r.traits.filter((e) => e.included && TF(e.name)), o = CU(i), s = wU(n, a), c = Math.max(o.value, s.value) + 1;
	...
	let l = eV((s.contribution ? bU(t, s.contribution) : null) ?? await oV("Armour", ["trait"]), "Armour", "trait");
...
function TU(e) {
	return e.type === "trait" && TF(e.name);
}
//#endregion
//#region src/module/apps/npc-builder/foundry-bridge/mounts/profile.ts
var EU = new Set(Object.values(SF));
async function DU(e) {
	return OU(iE(await fromUuid(e)));
}
function OU(e) {
	return {
		characteristics: {
...
function kU(e) {
	return e.items?.contents.filter((e) => e.type === "trait" && !FU(e)).map((t) => AU(e, t)).sort((e, t) => e.name.localeCompare(t.name)) ?? [];
}
function AU(e, t) {
	let n = Rl(t.system, [["rollable", "damage"]]), r = X(t.system, ["specification", "value"]);
	return {
		damage: n,
		fixedDamage: n ? jU(e, t, r) : null,
		name: t.name,
		specification: r,
		uuid: t.uuid
	};
...
function jU(e, t, n) {
	...
	return (Number.isFinite(i) ? i : 0) + (a ? NU(e, a) : 0);
}
function MU(e, t) {
	return Il(e.system, [[
		"characteristics",
...
async function LU(e, t) {
	let n = iE(await fromUuid(t));
	if (e.uuid === n.uuid) throw Error("The rider and mount must be different Actors.");
	let r = OU(e), i = OU(n), a = FF(r, i);
	await e.update(RU(e, a));
	let o = yU(n, a);
	o.length && await e.createEmbeddedDocuments("Item", o), await SU(e, n, i, a), await e.createEmbeddedDocuments("Item", [zF({
		flagScope: $,
		mount: i,
		plan: a,
		rider: r
...
function RU(e, t) {
	...
	return {
		...
		"system.characteristics.i.modifier": zU(e, "i") + t.initiative - BU(e, "i"),
		"system.characteristics.t.modifier": zU(e, "t") + t.toughness - BU(e, "t"),
...
function VU() {
	return game.actors.contents.map(CH).sort((e, t) => e.name.localeCompare(t.name));
}
//#endregion
...
async function HU(e) {
	...
	let t = await OH(e.careers), n = await WU(e);
	if (!n) throw Error("Foundry did not create the NPC Actor.");
	let r = GU(e), i = e.careers.at(-1), a = {
		name: r,
	...
	return c && (a.img = c, a["prototypeToken.texture.src"] = c), await n.update(a), await kH(n, t), await SV(n, e.advancements), await zV(n, e.traits), e.mountActorUuid && await LU(n, e.mountActorUuid), await uH(n, e.trappings), await mU(n, e.spells), n.sheet?.render(!0), ui.notifications?.info(`Created NPC "${r}".`), {
		name: r,
		uuid: n.uuid
	};
}
...
async function WU(e) {
	let t = iE(await fromUuid(e.baseActorUuid)).toObject(), n = $V(e.settings.outputActorFolderUuid);
	return delete t._id, delete t.folder, t.type = "npc", UV(t), n && (t.folder = n.id), await Actor.create(t);
}
function GU(e) {
	if (!e.settings.includeSpeciesInName) return e.actorName;
	let t = game.actors.contents.find((t) => t.uuid === e.baseActorUuid), n = t ? wH(t) : "";
...
async function KU(e) {
	...
	let n = aE(await fromUuid(t.uuid), "Drop a Foundry Item.");
...
function JU(e, t) {
	...
	if (tW(t.name, n, e.searchTerms)) for (let r of n) {
		let n = {
			img: r.path,
			key: `foundry-asset:${t.sourceKey}:${r.label}`,
			label: `${t.name || QU(r.path)} ${r.label} (${t.sourceLabel})`,
			source: "foundry-asset",
...
function eW(e) {
	return $U(e) && Object.values(e).every((e) => Array.isArray(e) && e.every((e) => typeof e == "string"));
}
function tW(e, t, n) {
	return qk(e, n) || t.some(({ path: e }) => qk(e, n));
}
function nW(e, t) {
	return Jk(e, {
...
var rW = "fuzzy-foundry", iW = .3, aW = new Set([
	".webp",
	".png",
	".jpg",
	".jpeg",
...
function sW() {
	let e = game.modules.get(rW)?.active === !0, t = cW(), n = fW(), r = Object.values(n?._fileIndexCache ?? {}).reduce((e, t) => e + t.length, 0);
	return {
		digDownActive: e,
...
function fW() {
	...
	return eW(t) && (r._fileIndexCache = t), $U(n) && typeof n.get == "function" && (r.fs = { get: n.get.bind(n) }), r;
}
function pW(e) {
	let t = `.${e.split(/[#?]/)[0]?.split(".").pop() ?? ""}`;
	return aW.has(t.toLocaleLowerCase());
}
//#endregion
...
function mW(e, t) {
	...
	for (let t of game.actors.contents) JU(e, {
		imagePaths: [{
			label: "actor image",
			path: t.img ?? ""
		}, {
	...
	for (let t of game.items?.contents ?? []) JU(e, {
		imagePaths: [{
			label: "item image",
			path: t.img ?? ""
		}],
...
async function gW(e, t) {
	...
	return e.includeCompendiumAssets && (mW(n, t), await hW(n, t)), e.includeFilePickerAssets && oW(n, t), YU(t, n, {
...
var _W = {
	buildNpc: HU,
	ensureActorFolder: YV,
	ensureItemFolder: XV,
	findLowerCareerCandidates: OB,
	getPortraitSearchAvailability: async () => sW(),
	importRecommendedQuickTraits: iH,
	listActorFolders: async () => ZV(),
	listBaseActors: async (e) => bH(e),
	listFoundryPortraitCandidates: gW,
	listMagicLoreOptions: NH,
	listMountActors: async () => VU(),
	listSpellsForMagicGrants: hU,
	listItemFolders: async () => QV(),
	listQuickTraits: aH,
	listSkillCharacteristics: WB,
	listSkillSpecializations: UB,
	listTalentMaximums: sV,
	listTraitDifficultyOptions: KV,
	loadBaseActorDraftData: xH,
	loadActorCombatProfile: DU,
	loadSettings: async () => _V(),
	resolveActorDrop: SH,
	resolveApplicationDrop: KU,
	resolveCareerDrop: DH,
	resolveSpellDrop: gU,
	resolveTraitDrop: qV,
	resolveTrapping: dH,
	resolveTrappingDrop: fH,
	saveSettings: vV
}, vW = class extends Nw {
	static DEFAULT_OPTIONS = {
		...super.DEFAULT_OPTIONS,
...
}, yW = "wfrp4e-customizer-open-npc-builder", bW = "wfrp4e-customizer-open-species-builder";
function xW() {
...
function CW(e, t) {
	...
	n.classList.add(yW, "wfrp4e-customizer-actor-directory-button"), n.type = "button", n.innerHTML = "<i class=\"fa-solid fa-user-plus\" inert></i><span>NPC Builder App</span>", n.addEventListener("click", () => {
		new vW().render(!0);
	}), t.append(n);
}
function wW(e, t) {
	if (e.querySelector(`.${bW}`)) return;
	let n = document.createElement("button");
	n.classList.add(bW, "wfrp4e-customizer-actor-directory-button"), n.type = "button", n.innerHTML = "<i class=\"fa-solid fa-people-group\" inert></i><span>Species Builder</span>", n.addEventListener("click", () => {
		new _O().render(!0);
	}), t.append(n);
}
function TW(e) {
...
function EW(e) {
	return e instanceof HTMLElement ? e : DW(e) && e[0] instanceof HTMLElement ? e[0] : null;
}
function DW(e) {
...
function kW(e, t) {
	return e.items?.contents.filter((e) => e.type === t).map((e) => ({
		name: e.name,
		value: t === "skill" ? AW(e.toObject().system) : jW(e.toObject().system)
	})) ?? [];
}
function AW(e) {
...
async function NW(e, t, n) {
	let r = game.actors.contents, i = PW(n ? r.filter((e) => e.folder?.uuid === n) : [], e);
	if (i) return {
		actor: i,
		source: i.folder?.name ?? "Configured NPC Base Actors folder"
	};
	let a = PW(r.filter((e) => e.uuid !== t.uuid), e);
	if (a) return {
		actor: a,
		source: "World Actors"
	};
	...
	if (!zW(s)) throw Error(`The species Actor ${o.uuid} is no longer available.`);
	return {
		actor: s,
		source: o.source
	};
...
function IW(e, t, n) {
	let r = t.trim();
	return e.find((e) => n(e).trim() === r) ?? e.find((e) => TO(n(e)) === TO(t)) ?? null;
}
function LW() {
	return MW ??= RW(), MW;
}
...
async function BW(e) {
	let t = iE(await fromUuid(e), "Expected an NPC Actor.");
...
var HW = "[data-wfrp-customizer-npc-xp=\"true\"]", UW = /* @__PURE__ */ new Set(), WW = !1, GW = !1;
function KW() {
	if (!WW) {
		WW = !0, Hooks.on("renderApplicationV2", (e, t) => {
...
function qW(e, t) {
	...
	if (!n) return;
	n.querySelector(HW)?.remove();
	let r = JW(e, t), i = n.querySelector(".sheet-list.careers");
	i ? n.insertBefore(r.container, i) : n.append(r.container), $W(), YW(r), globalThis.setTimeout(() => {
		r.root.isConnected && r.root.contains(r.container) && ($W(), UW.add(r));
	}, 0);
}
function JW(e, t) {
	let n = document.createElement("div");
...
async function YW(e) {
	...
	try {
		let t = await VW(e.actor);
		n === e.generation && e.root.contains(e.container) && XW(e, t);
	} catch (r) {
...
function ZW(e) {
	...
	let t = "actor" in e ? e.actor : void 0, n = "document" in e ? e.document : void 0, r = nE(t) ? t : nE(n) ? n : null;
	return r?.type === "npc" ? r : null;
}
function QW() {
	GW || (GW = !0, globalThis.setTimeout(() => {
		GW = !1, $W();
		for (let e of UW) YW(e);
	}, 0));
}
function $W() {
	for (let e of UW) (!e.root.isConnected || !e.root.contains(e.container)) && UW.delete(e);
}
//#endregion
...
function tG() {
	...
	let r = Yw(), i = new Set(J(n.species) ? Object.keys(n.species) : []), a = r.definitions.filter((e) => !i.has(e.key.trim()));
	rG(n, _l({
		...r,
		definitions: a
...
function rG(e, t) {
	for (let n of eG) iG(e, n, t[n]);
	aG(e, t.extraSpecies), oG(e, t.subspecies);
}
function iG(e, t, n) {
